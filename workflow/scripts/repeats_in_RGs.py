...
for item in gff:
    genes = [feature for feature in item.features if feature.type == "gene"]  # here we have IDs and positions
    item_id = item.id
    ...
    print("In record %s %i resistance genes not found" % (item_id, len(rgi_notLoose) - len(resistance_genes_coords)))
