...
```{r}
example_strains <- c("DA63084", "DA63186", "DA63322", "DA63946", "DA64026")
...
Nothing - no repeats around Isoniazid resistance genes

```{r}
rg_isoniazid <- filter(rg_df, grepl("isoniazid", Drug.Class))
rg_isoniazid <- select(rg_isoniazid, ORF_ID, strain, Drug.Class)

# unify gene names
repeat_counts_center2$record_id <- sub("_gene", "", repeat_counts_center2$record_id)
rg_isoniazid$record_id <- sapply(strsplit(rg_isoniazid$ORF_ID, " "), function(x){x[1]})

# join repeat counts
count_per_ab <- left_join(rg_isoniazid, repeat_counts_center2, by="record_id")
```
