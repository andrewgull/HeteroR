...
## Repeat number, length, AR length between R and S
...
Collect strains that have information on GM-resistance

```{r}
gm_res_df <- resistance_df %>% select(GM.SIR, strain) %>% filter(!is.na(GM.SIR))

gm_res_repeat_df_center <- inner_join(gm_res_df, repeat_df_center, by="strain")
gm_res_repeat_df_center
```

#### Repeat length

```{r}
ggplot(gm_res_repeat_df_center, aes(GM.SIR, length)) +
...
```{r}
ggplot(gm_res_repeat_df_center, aes(GM.SIR, AR_length)) +
...
```{r, message=F}
gm_res_rep_count <- gm_res_repeat_df_center %>%
  group_by(GM.SIR, strain) %>%
  summarize(n=n()) %>%
  filter(GM.SIR != "I")  # there are too little I's

ggplot(gm_res_rep_count, aes(GM.SIR, n)) +
  geom_boxplot(varwidth = T, notch = T)
```
