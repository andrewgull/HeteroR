...
ab_groups <- c("[P,p]enicillin[s]", "[C,c]ephalosporin[s]", "[T,t]etracycline[s]", "[A,a]minoglycoside[s]", "[M,m]acrolide[s]", "[C,c]lindamycin", "[S,s]ulfonamide[s]", "[T,t]rimethoprim", "[M,m]etronidazole", "[T,t]inidazole", "[Q,q]uinolone[s]", "[N,n]itrofurantoin")

for (group in ab_groups){
  x <- filter(rg_df_notLoose, grepl(group, Drug.Class)) %>% nrow()
  print(x)
}
```
...
Let's get some repeats around Macrolide-resistance genes (from notLoose table)

```{r}
rg_ab <- filter(rg_df_notLoose, grepl("macrolide", Drug.Class))
