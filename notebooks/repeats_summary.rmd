...
## AR length per AB group all strains

### Join tables

```{r}
# join repeat_df_center and rg_ab_found_all by record_id
# prepare record_id in repeat_df_center
repeat_df_center$record_id <- sub("_gene", "", repeat_df_center$record_id)
# join
repeat_df_center_ab_ar <- left_join(repeat_df_center, rg_ab_found_df_all, by="record_id") %>% filter(!is.na(Drug.Class))
...
```{r, fig.width=16, fig.height = 8, message = FALSE, warning = FALSE}
box.ar.ab.all <- plot_ly(data = repeat_df_center_ab_ar,
                      x = ~strain.x,
                      y = ~AR_length,
                      color = ~AB.group,
                      type = "box") %>%
