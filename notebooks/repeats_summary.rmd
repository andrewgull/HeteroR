...
Read the data, currently it contains 370 strains
...
To get different strains, go to shiny_apps
...
## Mean number of repeats per AB group
...
some repeats around Macrolide resistance genes
...
```{r}
ggplot(count_per_ab, aes(strain.x, n_repeats)) +
        geom_boxplot(alpha=0.5, outlier.size = 1.0, outlier.alpha = 0.5, size=0.2, varwidth=TRUE) +
        geom_violin(alpha=0.2, size=0.2) +
        coord_trans(y="sqrt") +
        theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) +
        ylab("n") +
        xlab("") +
        ggtitle("repeats count around macrolide resistance genes")
```
