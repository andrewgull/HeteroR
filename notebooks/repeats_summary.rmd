...
This notebook relies on results from *make_repeat_tables.py* that creates tables which contain coordinates of repeat for each strain and each resistance gene
...
# Ananlysis of repeat pairs in various AB groups

Here you find repeat statistics in AB groups.
...
**OBS!** Evoking this filtering will remove majority found AB groups
...
**N.B.** The following table is made with an assumption that entries in *Drug.Class* contain the following names of AB groups:
...
ggbox.rl.ab.100 <- ggplot(ab_res_repeat_df %>% filter(length >= 100), aes(AB , length)) +
  ...
  facet_grid(spans_center ~ .) +
  scale_fill_brewer(palette = "Set2") +
  ggtitle("Repeat length", subtitle="Repeats shorter than 100 bp were removed")+
  ylab("length") +
  xlab("antibiotic")
