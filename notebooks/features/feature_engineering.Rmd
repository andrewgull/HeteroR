...
### Add tidy version to filtered features
...
features_amp_strain <- features_amp %>% select(strain, n.plasmids, AB, resistance) %>% distinct()
...
gff_amp$on.plasmid <- if_else(gff_amp$located.on.plasmid, 1, 0)
```
...
## Add 'on.plasmid' to `features_amp` 

```{r}
features_amp <- left_join(features_amp, gff_amp, by="record_id")

```


## Add 'strain' and 'record_id' to GFF
...
features_amp_strain <- left_join(features_amp_strain, repeat_counts, by="strain")


# count number of repeats spanning center
rep_counts_center <- features_amp %>% 
...
features_amp_strain <- left_join(features_amp_strain, rep_counts_center, by="strain")
...
features_amp <- left_join(features_amp, repeat_counts_gene, by=c("record_id", "on.plasmid"))
...
## Make BL type counts per strain for the heatmap
...
## Add BL-type counts to features_amp_strain

```{r}
features_amp_strain <- left_join(features_amp_strain, amp_amr_types_strain, by = "strain") 
```
...
# features_amp_strain <- left_join(features_amp_strain, features_amp %>% select(strain, n.plasmids) %>% distinct(), by="strain") %>% relocate(n.plasmids, .before="n.rep.total")
features_amp_strain <- features_amp_strain %>% 
  filter(resistance != "I", !is.na(resistance)) %>% relocate(resistance, .before = n.plasmids) %>% select(-AB)
features_amp_strain
```
...
write.csv(amp_amr_types_strain, "/home/andrei/Data/HeteroR/results/tables/amp_amr_types_strain.csv", row.names = FALSE)
```

# CFX tested strains

CFX - cefotaxime acid, cephalosporine class.
...
```{r}
# filter those strains that were tested for CFX
# keep for per gene feature calculations
features_cfx <- features_filtered %>% filter(AB=="CFX") %>% distinct()
# this df is for per strain calculation
features_cfx_strain <- features_cfx %>% select(strain, n.plasmids, AB, resistance) %>% distinct()
head(features_cfx_strain)
```
...
```{r}
f_cfx_filt <- features_cfx %>% 
  select(strain, record_id, AMR.Gene.Family) %>% 
  distinct()

f_cfx_filt$is_beta_lac <- grepl("beta-lactamase", f_cfx_filt$AMR.Gene.Family)

n.beta.lac <- f_cfx_filt %>% group_by(strain) %>% summarise(n.beta.lac=sum(is_beta_lac))

# join with features_cfx_strain
features_cfx_strain <- left_join(features_cfx_strain, n.beta.lac, by="strain")

# add is beta lac to features cfx for future calculations
features_cfx <- left_join(features_cfx, f_cfx_filt %>% select(record_id, is_beta_lac), by="record_id") %>% 
  relocate(is_beta_lac, .before="AB")

head(features_cfx_strain)
```

## Filtering AMR Gene Family

I have 21 gene families among CFX tested strains. 

What genes should I use for features construction?

```{r}
unique(f_cfx_filt$AMR.Gene.Family)
```
