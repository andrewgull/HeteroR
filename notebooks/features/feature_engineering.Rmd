...
n_genes_plus_strand <- gff_bl %>% 
  group_by(strain, on.plasmid) %>% 
  summarise(n.genes.plus=sum(pos.strand)) %>% 
  spread(key = "on.plasmid", value = "n.genes.plus", fill = 0) %>% 
  rename("strain" = strain, "n.genes.plus.chrom" = `0`, "n.genes.plus.plasmid" = `1`) %>% 
  mutate(n.genes.plus = n.genes.plus.chrom + n.genes.plus.plasmid)
