...
```{r, eval=FALSE}
repeat_df <- read_csv("/home/andrei/Data/HeteroR/results/tables/repeats_summary.csv") %>% 
  select(-X) %>% 
  relocate(strain, .before=record_id)
...
### Read and process AB testing table (PTZ)

HR testing data against PIP-TAZO
...
resistance_df <- read_csv("/home/andrei/Data/HeteroR/resources/heteroresistance_testing_ptz.csv")
...
```{r}
resistance_df %>% group_by(resistance) %>% summarize(N=n())
```


### Add resistance testing to filtered features

```{r}
features_filtered <- left_join(features_filtered, resistance_df, by="strain")
features_filtered
```

### Plot: length of repeats around AMR genes

On average, repeat length around AMR genes is greater in HR group

```{r}
ggplot(features_filtered %>% filter(!is.na(resistance)), aes(resistance, repeat_length)) +
  geom_boxplot(aes(fill = resistance), alpha = 0.5, varwidth = T, outlier.size = 0.6, outlier.alpha = 0.2, notch = T) +
  geom_violin(aes(fill = resistance), alpha = 0.3) +
  coord_trans(y = "log") +
...
```{r}
ggplot(features_filtered %>% filter(!is.na(resistance)), aes(resistance, AR_length)) +
  geom_boxplot(aes(fill = resistance), alpha = 0.5, varwidth = T, outlier.size = 0.6, outlier.alpha = 0.2, notch = T) +
  geom_violin(aes(fill = resistance), alpha = 0.3) +
  coord_trans(y = "sqrt") +
  scale_fill_brewer(palette = "Set1")+
...
features_filtered$n.plasmids <- ifelse(is.na(features_filtered$n.plasmids), 0, features_filtered$n.plasmids)
features_filtered <- features_filtered %>% relocate(n.plasmids, .before="ORF_ID")
...
ggpubr::ggarrange(h1, h2, ncol=2, nrow=1)
```
...
features_bl_strain <- features_filtered %>% select(strain, n.plasmids, resistance) %>% distinct()
...
## Add 'strain' and 'record_id' to GFF with BL genes
...
### Summarize some GFF features per strain
...
med_dist_oriC <- gff_bl %>% group_by(strain) %>% 
  summarise(med.dist.oriC=median(dist.to.oriC))

n_genes_plus_strand <- gff_bl %>% group_by(strain) %>% 
  summarise(n.genes.plus.strand=sum(pos.strand))

n_genes_plasmids <- gff_bl %>% group_by(strain) %>% 
...
# NAs to 0s
features_bl_strain$n.rep.100 <- ifelse(is.na(features_bl_strain$n.rep.100), 0, features_bl_strain$n.rep.100)
...
# replace NAs with 0s
features_bl_strain$n.rep.100.cen <- ifelse(is.na(features_bl_strain$n.rep.100.cen), 0, features_bl_strain$n.rep.100.cen)

## non spanning center
features_bl_strain$n.rep.100.non.cen <- features_bl_strain$n.rep.100 - features_bl_strain$n.rep.100.cen
...
repeat_counts_strain_500 <- features_filtered %>% 
  filter(is_beta_lac) %>%
...
# add
features_bl_strain <- left_join(features_bl_strain, repeat_counts_strain_500, by="strain")
# NAs to 0s
features_bl_strain$n.rep.500 <- ifelse(is.na(features_bl_strain$n.rep.500), 0, features_bl_strain$n.rep.500)

# n repeats longer than 500 and spanning center
repeat_counts_strain_500_center <- features_filtered %>% 
  filter(is_beta_lac, spans_center=="yes", repeat_length >= 500) %>%
  group_by(strain) %>% 
  summarise("n.rep.500.cen"=n())

# add to the table, there will be NAs
features_bl_strain <- left_join(features_bl_strain, repeat_counts_strain_500_center, by="strain")
# replace NAs with 0s
features_bl_strain$n.rep.500.cen <- ifelse(is.na(features_bl_strain$n.rep.500.cen), 0, features_bl_strain$n.rep.500.cen)

## non spanning center
features_bl_strain$n.rep.500.non.cen <- features_bl_strain$n.rep.500 - features_bl_strain$n.rep.500.cen
...
features_bl_strain <- left_join(features_bl_strain, rep_len_sum_strain, by="strain")
...
med_rep_len_center <- features_filtered %>% 
  filter(is_beta_lac, spans_center=="yes") %>% 
  select(strain, repeat_length) %>% 
  group_by(strain) %>% 
...
# non spanning
med_rep_len_non_center <- features_filtered %>% 
...
# add
features_bl_strain <- left_join(features_bl_strain, med_rep_len_center, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_rep_len_non_center, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_rep_len, by="strain") # features_bl_strain <- features_bl_strain %>% relocate(med.rep.len, .before = "med.rep.len.cen")


# NAs to 0s
features_bl_strain$med.rep.len.cen <- ifelse(is.na(features_bl_strain$med.rep.len.cen), 0, features_bl_strain$med.rep.len.cen)
features_bl_strain$med.rep.len.non.cen <- ifelse(is.na(features_bl_strain$med.rep.len.non.cen), 0, features_bl_strain$med.rep.len.non.cen)

# older way of counting
# med_rep_len_old <- features_filtered %>% 
#   filter(is_beta_lac) %>% 
#   select(strain, repeat_length) %>% 
#   distinct() %>% 
#   group_by(strain) %>% 
...
```{r}
med_rep_len_m100 <- features_filtered %>% 
...
# spanning gene center
med_rep_len_center_m100 <- features_filtered %>% 
  filter(is_beta_lac, spans_center=="yes", repeat_length >= 100) %>% 
  select(strain, repeat_length) %>% 
  group_by(strain) %>% 
...
# non spanning
med_rep_len_non_center_m100 <- features_filtered %>% 
...
# add
features_bl_strain <- left_join(features_bl_strain, med_rep_len_m100, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_rep_len_center_m100, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_rep_len_non_center_m100, by="strain")

# replace NAs
features_bl_strain$med.rep.len.100 <- ifelse(is.na(features_bl_strain$med.rep.len.100), 0, features_bl_strain$med.rep.len.100)
features_bl_strain$med.rep.len.cen.100 <- ifelse(is.na(features_bl_strain$med.rep.len.cen.100), 0, features_bl_strain$med.rep.len.cen.100)
features_bl_strain$med.rep.len.non.cen.100 <- ifelse(is.na(features_bl_strain$med.rep.len.non.cen.100), 0, features_bl_strain$med.rep.len.non.cen.100)
...
```{r}
max_rep_len_all <- features_filtered %>% group_by(strain) %>% summarize(max.rep.len=max(repeat_length, na.rm = T))

max_rep_len_cen <- features_filtered %>% filter(spans_center=="yes") %>% group_by(strain) %>% summarize(max.rep.len.cen=max(repeat_length, na.rm = T))

max_rep_len_non_cen <- features_filtered %>% filter(spans_center=="no") %>% group_by(strain) %>% summarize(max.rep.len.non.cen=max(repeat_length, na.rm = T))

# add
features_bl_strain <- left_join(features_bl_strain, max_rep_len_all, by="strain")
features_bl_strain <- left_join(features_bl_strain, max_rep_len_cen, by="strain")
features_bl_strain <- left_join(features_bl_strain, max_rep_len_non_cen, by="strain")

# NAs to 0
features_bl_strain$max.rep.len.cen <- ifelse(is.na(features_bl_strain$max.rep.len.cen), 0, features_bl_strain$max.rep.len.cen)
...
med_ar_len_non_cen <- features_filtered %>% 
  filter(is_beta_lac, spans_center=="no") %>% 
  select(strain, AR_length) %>% 
  group_by(strain) %>% 
...
# add
features_bl_strain <- left_join(features_bl_strain, med_ar_len, by="strain") # relocate(features_bl_strain, med.AR.len, .before="med.AR.len.cen")
features_bl_strain <- left_join(features_bl_strain, med_ar_len_cen, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_ar_len_non_cen, by="strain")
...
```{r}
med_ar_len_m100 <- features_filtered %>% 
  filter(is_beta_lac, repeat_length >= 100) %>% 
  select(strain, AR_length) %>% 
  group_by(strain) %>% 
  summarize(med.AR.len.100=median(AR_length, na.rm = T))

med_ar_len_cen_m100 <- features_filtered %>% 
  filter(is_beta_lac, spans_center=="yes", repeat_length >= 100) %>% 
  select(strain, AR_length) %>% 
  group_by(strain) %>% 
  summarize(med.AR.len.cen.100=median(AR_length, na.rm = T))

med_ar_len_non_cen_m100 <- features_filtered %>% 
  filter(is_beta_lac, spans_center=="no", repeat_length >= 100) %>% 
  select(strain, AR_length) %>% 
  group_by(strain) %>% 
...
# add
# NA can appear
features_bl_strain <- left_join(features_bl_strain, med_ar_len_m100, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_ar_len_cen_m100, by="strain")
features_bl_strain <- left_join(features_bl_strain, med_ar_len_non_cen_m100, by="strain")

# replace NAs with 0
features_bl_strain$med.AR.len.cen.100 <- ifelse(is.na(features_bl_strain$med.AR.len.cen.100), 0, features_bl_strain$med.AR.len.cen.100)
features_bl_strain$med.AR.len.non.cen.100 <- ifelse(is.na(features_bl_strain$med.AR.len.non.cen.100), 0, features_bl_strain$med.AR.len.non.cen.100)
features_bl_strain$med.AR.len.100 <- ifelse(is.na(features_bl_strain$med.AR.len.100), 0, features_bl_strain$med.AR.len.100)
...
features_bl_strain <- left_join(features_bl_strain, med_rep_len_ampC, by="strain")
...
features_bl_strain <- left_join(features_bl_strain, max_rep_len_all_ampC, by="strain")
...
features_bl_strain <- left_join(features_bl_strain, rep_len_sum_ampC, by="strain")
...
features_bl_strain <- left_join(features_bl_strain, med_ar_len_ampC, by="strain")
...
features_bl_strain <- left_join(features_bl_strain, med_ar_len_cen_ampC, by="strain")
features_bl_strain$ampC.med.AR.len.cen <- ifelse(is.na(features_bl_strain$ampC.med.AR.len.cen), 0, features_bl_strain$ampC.med.AR.len.cen)

```

## Make AMR genes counts per strain for the heatmap
