...
`features_amp` contains gene names, so I can find distance to *oriC* in circularized genomes.
...
Since some strains will get NA here, it will work only with algorithms that can handle missing observations.
...
gff <- as.tibble(read.delim("/home/andrei/Data/HeteroR/results/annotations/DA63072/prokka/DA63072_genomic.gff", header=F, comment.char="#")) %>% select(V1, V3, V4, V5, V7, V9) %>% filter(V3=="gene")

names(gff) <- c("seq.region", "type", "start", "stop", "strain", "ID")
