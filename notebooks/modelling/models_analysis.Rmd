...
lr_base_res %>% 
  ...
  filter(.config == "Preprocessor1_Model26")
```
...
lsvm_ncorr_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == "Preprocessor06_Model1")
```
...
lsvm_ncoryj_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == "Preprocessor10_Model1")
```
...
psvm_ncorr_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == "Preprocessor05_Model1")
```
...
psvm_pca_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == "Preprocessor2_Model05")
```
...
rbf_ncorr_res %>% 
  show_best(metric = "j_index") %>% 
  filter(.config == "Preprocessor19_Model1")
```
...
rbf_ncoryj_res %>% 
  show_best(metric = "j_index", n = 15) %>% 
  filter(.config == "Preprocessor11_Model1")
```
...
rf_base_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == "Preprocessor1_Model28")
```
...
mlp_ncor_res %>% 
  show_best("j_index") %>% 
  filter(.config == "Preprocessor13_Model1")
```
...
mlp_pca_res %>% 
  show_best("j_index") %>% 
  filter(.config == "Preprocessor2_Model03")
```
...
mlp_bag_ncorr_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == "Preprocessor09_Model1")
```
...
mlp_bag_pca_res %>% 
  show_best(metric = "j_index", n = 15) %>% 
  filter(.config == "Preprocessor3_Model4")
```
...
The dashed line this figure shows the 1-AUC for the full model. Features farther to the right are more important, because permuting them results in higher AUC.
