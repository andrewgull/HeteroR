...
```{r}
# function for MAE
mae_4bl_model <- function(predictions) {
  predictions %>% 
  mutate(pred_score = if_else(pred == "nonHR", 0, 1),
         resistance_score = if_else(resistance == "nonHR", 0, 1)) %>% 
  mae(truth = resistance_score, estimate = pred_score)
}
```
...
```{r}
lr_base_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == get_config_name(lr_base_res))
```
...
lr_base_yj_res %>% 
  ...
  filter(.config == best_config_name(lr_base_yj_res))
```
...
lr_base_orq_res %>% 
  show_best(metric = "j_index") %>% 
  filter(.config == best_config_name(lr_base_orq_res))
```
...
lsvm_ncorr_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(lsvm_ncorr_res))
```
...
lsvm_ncoryj_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(lsvm_ncoryj_res))
```
...
lsvm_ncorq_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(lsvm_ncorq_res))
```
...
```{r}
lsvm_pca_res %>%
  show_best(metric = "j_index", n=50) %>% 
  filter(.config == best_config_name(lsvm_pca_res))
```
...
psvm_ncorr_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(psvm_ncorr_res))
```
...
```{r}
psvm_ncoryj_res %>%
  show_best(metric = "j_index", n = 15) %>% 
  filter(.config == best_config_name(psvm_ncoryj_res))
```
...
psvm_ncorq_res %>% 
  show_best("j_index") %>% 
  filter(.config == best_config_name(psvm_ncorq_res))
```
...
```{r}
psvm_pca_res %>% 
  show_best("j_index", n = 50) %>% 
  filter(.config == best_config_name(psvm_pca_res))
```
...
rbf_ncorr_res %>% 
  show_best(metric = "j_index") %>% 
  filter(.config == best_config_name(rbf_ncorr_res))
```
...
rbf_ncoryj_res %>% 
  show_best(metric = "j_index", n = 15) %>% 
  filter(.config == best_config_name(rbf_ncoryj_res))
```
...
rbf_ncorq_res %>% 
  show_best(metric = "j_index") %>% 
  filter(.config == best_config_name(rbf_ncorq_res))
```
...
rbf_pca_res %>% 
  show_best(metric = "j_index", n = 15) %>% 
  filter(.config == best_config_name(rbf_pca_res))
```
...
rf_base_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(rf_base_res))
```
...
rf_base_bor_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(rf_base_bor_res))
```
...
bt_base_res %>% 
  show_best(metric = "j_index", n = 50) %>% 
  filter(.config == best_config_name(bt_base_res))
```
...
mlp_ncor_res %>% 
  show_best("j_index") %>% 
  filter(.config == best_config_name(mlp_ncor_res))
```
...
mlp_ncoryj_res %>% 
  show_best("j_index") %>% 
  filter(.config == best_config_name(mlp_ncoryj_res))
```
...
mlp_ncorq_res %>% 
  show_best(metric = "j_index") %>% 
  filter(.config == best_config_name(mlp_ncorq_res))
```
...
mlp_pca_res %>% 
  show_best("j_index") %>% 
  filter(.config == best_config_name(mlp_pca_res))
```
...
```{r}
show_best(mlp_bag_ncorr_res, "roc_auc")
```

```{r}
mlp_bag_ncorr_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(mlp_bag_ncorr_res))
```
...
mlp_bag_ncoryj_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(mlp_bag_ncoryj_res))
```
...
mlp_bag_ncorq_res %>% 
  show_best("j_index", n = 15) %>% 
  filter(.config == best_config_name(mlp_bag_ncorq_res))
```
...
```{r}
mlp_bag_pca_res %>% 
  show_best(metric = "j_index", n = 25) %>% 
  filter(.config == best_config_name(mlp_bag_pca_res))
```
...
probabilistic error interpretation

HR = 1, non-HR = 0

then use standard MAE formula

```{r}
lr_final[[2]] %>%
  collect_predictions() %>% 
  mutate(score = if_else(resistance == "HR", 1, 0)) %>% 
  mae(truth = score, estimate = .pred_HR)
```
...
This metric doesn't depend on the classification threshold.
...
lr_coeff_df <- lr_final[[2]] %>%
  ...
  mutate(
    ...
    category = case_when(
      grepl("IS", term) |
        grepl("rep", term) |
        grepl("AR", term) | grepl(".is.", term, fixed = TRUE) ~ "repeats",
      grepl("beta", term) |
        grepl("transfer", term) |
        grepl("n.c.plasmids", term) |
        grepl("n.plasmids", term) |
        grepl("APH6", term) |
        grepl("AAC3", term) | 
        grepl("ANT3", term) | 
        grepl("APH3", term) |
        grepl(".dfr", term) | grepl(".sul", term) ~ "genes",
      grepl("linear", term) |
        grepl("read.max.len", term) |
        grepl("coverage", term) ~ "technical"
    ),
    # make readable term names
    term = recode(term,
                  "n.beta.lac" = "number of bla genes",
                   "TEM.beta.lactamase" = "number of bla TEM genes",
                   "ampC.type.beta.lactamase" = "number of bla ampC genes ",
                  "med.AR.len.TEM" = "median AR length around bla TEM genes",
                  "CTX.M.beta.lactamase" = "number of bla CTX-M genes",
                  ...
                  "min.distance.is.gene" = "distance between IS element and closest RG",
                  ...
                  "med.rep.len.ampC" = "median repeat length around bla ampC genes",
                  "n.beta.lac.plasmid" = "number of bla genes on plasmids",
                  ...
                  "med.AR.len.plasmid.TEM" = "median AR length around bla TEM genes on plasmids",
                  "med.rep.len.plasmid.TEM" = "median repeat length around bla TEM genes on plasmids",
                  ...
                  "n.rep.chrom.ampC" = "number of repeats around bla ampC genes on chromosomes",
                  ...
                  "APH3" = "number of APH3 genes",
                  "rep.tot.cen" = "number of repeat pairs spanning RG",
                  "med.AR.len.chrom.ampC" = "median AR length around bla ampC genes on chromosomes",
                  "n.plasmids" = "number of plasmids",
                  "n.rep.plasmid.ampC" = "number of repeats around bla ampC genes on plasmids",
                  "med.AR.len.plasmid.ampC" = "median AR length around bla ampC genes on plasmids",
                  "n.rep.100.cen.tot" = "number of repeat pairs longer than 100 nt spanning RG",
                  "n.rep.500.chrom" = "number of repeats longer than 500 nt on chromosomes",
                  "max.rep.len.plasmid" = "maximal repeat length on plasmdis",
                  "med.rep.len.chrom.ampC" = "median repeat length around bla ampC genes on chromosomes",
                  "coverage" = "Nanopore coverage",
                  "read.max.len" = "maximal Nanopore read length",
...
Fig_LR_predictors_alt <-
  ...
  scale_y_break(c(3, 29), ticklabels = c(30)) +
  theme(
    legend.position = "top",
    axis.text.x = element_text(
      vjust = 0.9,
...
ggsave(file="manuscript/images/Fig_LR_predictors_alt.png", 
       plot=Fig_LR_predictors_alt, width = 8, height=8, units = "in")
...
# 90 predictors
vip_gbt %>% group_by(variable) %>% summarise(m = median(dropout_loss))
```
...
# make readable predictors' names

lr_terms <- lr_final[[2]] %>%
  extract_fit_parsnip() %>% tidy() %>%
  filter(estimate != 0) %>% pull(term)

# setdiff(unique(vip_gbt$variable), lr_terms)
...
vip_gbt <- vip_gbt %>%
  mutate(
    variable = recode(
      ...
      "IS5" = "number of IS5 elements",
      "ANT3" = "number of ANT3 genes",
      "med.tot.rep.len.chrom" = "median repeat length on chromosomes",
      "ISNCY" = "number of ISNCY elements",
      "med.AR.len.cen" = "median AR length",
      "chloramphenicol.acetyltransferase.CAT" = "number of CAT genes",
      "n.is.tot" = "total number of IS families",
      ...
      "APH6" = "number of APH6 genes",
      "n.c.plasmids" = "copy number of plasmids",
      "coverage" = "Nanopore sequencing coverage",
      "min.AR.len.cen.plasmid" = "minimal length of AR spanning RG on plasmids",
      "read.max.len" = "length of the longest Nanopore read",
      "n.rep.plasmid.TEM" = "number of repeats arount bla TEM genes on plasmids",
      "med.rep.len.TEM" = "median length of repeats around bla TEM genes",
      "n.rep.500.total" = "total number of repeats longer than 500 nt",
      "med.AR.len.chrom.ampC" = "median length of AR around bla ampC genes on chromosomes",
      "n.rep.cen.plasmid" = "number of repeats spanning RG on plasmids",
      "med.tot.rep.len.plasmid" = "median total repeat length on plasmids",
      "med.rep.len.chrom.ampC" = "median length of repeats around bla ampC genes on chromosomes",
      "max.rep.len.cen.plasmid" = "maximal length of repeats spanning RG on plamids",
      "n.rep.plasmid" = "number of repeats on plasnids",
      "med.AR.len" = "median AR length",
      "n.rep.cen.chrom" = "number of repeats spanning RG on chromosomes",
      "med.rep.len" = "median repeat length",
      "med.AR.len.cen.chrom" = "median length of AR spanning RG on chromosomes",
      "n.rep.100.cen.tot" = "total number of repeats longer than 100 nt spanning RG",
      "n.rep.plasmid.ampC" = "number of repeats around bla ampC on plasmids",
      "med.AR.len.chrom" = "median AR length on chromosomes",
      "n.rep.tot.cen" = "total number of repeats spanning RG",
      "n.rep.100.chrom" = "number of repeats longer than 100 nt on chromosomes",
      "max.rep.len.chrom" = "maximal length of repeats on chromosomes",
      "n.rep.total" = "total number of repeats",
      "n.rep.500.plasmid" = "number of repeats longer than 500 nt on plasmids",
      "med.rep.len.plasmid.ampC" = "median length of repeats around bla ampC genes on plasmids",
      "med.AR.len.ampC" = "median length of AR around bla ampC genes",
      "APH3" = "number of APH3 genes",
      "med.AR.len.plasmid.ampC" = "median length of AR around ampC genes on plasmids",
      "n.rep.chrom" = "number of repeats on chromosomes",
      "n.plasmids" = "number of plasmids",
      "max.rep.len.cen.chrom" = "maximal length of repeats spanning RG on chromosomes",
      "n.rep.100.cen.plasmid" = "number of repeats longer than 100 nt spanning RG on plasmids",
      "max.rep.len.plasmid" = "maximal length of repeats on plasmids",
      "med.rep.len.plasmid" = "median length of repeats on plasmids",
      "n.rep.500.chrom" = "number of repeats longer than 500 nt on chromosomes",
      "min_distance_is_gene" = "minimal distance between IS and the nearest RG",
      "rep.tot.cen" = "number of repeat pairs spanning genes",
      "n.beta.lac.chrom" = "number of bla genes on chromosomes",
      "n.TEM.1" = "number of TEM-1 genes",
      "chrom.status_linear" = "incomplete chromosome assembly"
  )
)
...
The dashed line this figure shows the 1-AUC for the full model. Features farther to the right are more important, because permuting them results in dropped AUC.
