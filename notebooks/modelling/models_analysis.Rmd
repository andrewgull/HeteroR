...
# path for models
models_path <- "/mnt/data/andrei/Data/HeteroR/results/models/scheme12/sept/"
```
...
looks like the n of components selcted is 1. lets check 
...
best_perf_hyp <- lr_pca_res %>% show_best(n = 20)
...
my_mod <-  logistic_reg(penalty =  best_perf_hyp$penalty[1],
      ...
      n_predictors <-
        coefficients[coefs@i] %>% purrr::discard( ~ .x %in% predictors_to_remove) %>% length
...
```{r}
Fig_13A <- senspec_plot(lr_final[[2]], "LLR") 
...
```{r}
Fig_13B <-senspec_plot(lgb_min_final[[2]], "LGB top 5")
Fig_13B
```

```{r, fig.width=10,fig.height=5}
Fig_13AB <- ggarrange(Fig_13A, Fig_13B+ ylab(""),
                    labels = c("A", "B"),
                    ncol = 2, nrow = 1, common.legend = TRUE,
                    legend = "bottom")
Fig_13AB
...
cm_optimized_lgb_min <- optimize_prediction(lgb_min_final[[2]]) %>% 
  conf_mat(truth = resistance, estimate = .pred) %>% 
  autoplot(type = "heatmap") + ggtitle("LR")

cm_optimized_lgb_min
```
...
```{r opt perf}
  ...
  get_metrics2(lgb_min_final[[2]], "lgb_min") %>% 
  pivot_wider(names_from = model, values_from = .estimate) %>% 
  select(-.estimator)
```
