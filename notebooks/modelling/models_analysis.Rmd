...
Just 42 features plus Intercept are used by the model.
...
df_train_proc <- ncorr_yj_recipe_tuned %>% prep() %>% juice()
...
explainer_final <- 
  explain_tidymodels(
    mlp_fit, 
    data = df_train_proc, 
    y = df_train_proc$resistance,
    label = "MLP",
    verbose = FALSE
  )

# to get an observation from the test set, you'll need processed test data
obs_interest <- df_train_proc %>% 
  filter(strain == "DA62886") 
