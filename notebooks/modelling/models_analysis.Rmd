...
Alternative 1: categories of predictors

```{r}
# add categories of the predictors, i.e. genes, repeats or technical artefacts
Fig_LR_predictors_alt <- 
lr_coeff_df %>%
  mutate(
    HR_probability = if_else(estimate < 0, "increased", "decreased"),
    odds.ratio_ = if_else(odds.ratio < 1.0, odds.ratio * (-1), odds.ratio),
    category = case_when(
      grepl("IS", term) |
        grepl("rep", term) |
  ...
  filter(term != "(Intercept)") %>% 
  ggplot(aes(reorder(term, -odds.ratio, decreasing = T), odds.ratio_)) +
  geom_col(aes(fill = category)) +
  scale_fill_brewer(palette = "Dark2", guide = guide_legend(title = NULL), direction = -1) +
  #scale_fill_manual(values = c("#E77200", "#6A329F", "#0B6629"), guide = guide_legend(title = NULL)) +
  #scale_alpha_discrete(range = c(0.6, 1), guide = guide_legend(title = NULL)) +
  theme(
    axis.text.x = element_text(
      angle = 90,
      vjust = 0.9,
...
ggsave(file="manuscript/images/Fig_LR_predictors_alt.png", plot=Fig_LR_predictors_alt, width = 8, height=6, units = "in")
...
lr_coeff_df %>%
  ...
  filter(term != "(Intercept)", HR_probability == "increased") %>% 
  ggplot(aes(x = reorder(term, -odds.ratio, decreasing = T), y = odds.ratio_)) +
  geom_col(fill = "steelblue") +
  #geom_point(aes(alpha=0.5,  color="black")) +
  #scale_size(range = c(.1, 24), name="UU") +
  scale_fill_brewer(palette = "Dark2", guide = guide_legend(title = NULL), direction = -1) +
...
ggsave(file="manuscript/images/Fig_LR_predictors_alt2.png", plot=Fig_LR_predictors_alt2, width = 8, height=6, units = "in")
...
lr_coeff_df %>%
  ...
  filter(term != "(Intercept)", HR_probability == "decreased") %>% 
  ggplot(aes(x = reorder(term, -odds.ratio, decreasing = T), y = odds.ratio)) +
  geom_col(fill = "steelblue") +
  scale_fill_brewer(palette = "Dark2", guide = guide_legend(title = NULL), direction = -1) +
  theme(
    axis.text.x = element_text(
...
ggsave(file="manuscript/images/Fig_LR_predictors_alt3.png", plot=Fig_LR_predictors_alt3, width = 8, height=6, units = "in")
