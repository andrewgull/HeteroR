...
top_names <- c("LR", "GBT", "SVM polynomial")
...
psvm_spec_tuned <- svm_poly(
  cost = best_models[[3]]$cost,
  degree = best_models[[3]]$degree,
  scale_factor = best_models[[3]]$scale_factor
) %>% 
  set_mode("classification") %>%
  set_engine("kernlab", num.threads = 4)
...
rocs_final <-
  map2(list(lr_final[[2]], gbt_base_final[[2]]),
       mod_labels,
       ~ make_final_roc(.x, .y))
...
```{r}
mod_labels <- c("LR", "pSVM")

rocs_final <-
  map2(list(lr_final[[2]], psvm_ncoryj_final[[2]]),
       mod_labels,
       ~ make_final_roc(.x, .y))
...
conf_mat_final <- map2(list(lr_final[[2]], gbt_base_final[[2]]), mod_labels, ~make_cm(.x, .y))
...
Just 32 features plus Intercept are used by the model.
...
My probability threshold is 0.27, these strains are predicted as HR

```{r}
optimize_prediction(lr_final[[2]]) %>% filter(.pred_HR >= 0.27)
```
...
ggarrange(plotlist = plot_list, ncol=3, nrow=5, common.legend = T)
```
...
```{r}
minimal_pred_best <- minimal_pred %>% filter(pred == "n.rep.plasmid.TEM" |
...
## A BT model with the top 4 predictors
...
```{r}
make_final_roc(lgb_min_final[[2]], "LGB top 4")
```
...
```{r}
get_metrics(lgb_min_final[[2]], "LGB top 4") %>% pivot_wider(names_from = model, values_from = .estimate)
```

#### Final sensitivity/specificity plot

```{r}
senspec_plot(lgb_min_final[[2]], "LGB top 4")
```
