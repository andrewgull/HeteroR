...
There are two model objects published along with this code: `models/llr_final.rds` and `models/gbt_final.rds`

Use them in order to skip the training/validation/testing process.
...
```{r}
show_best(lsvm_pca_res, metric = "roc_auc", n = 3) %>% 
  kable()
```
...
```{r}
show_best(psvm_ncoryj_res, metric = "roc_auc", n = 3) %>% 
  kable()
```
...
```{r}
show_best(rbf_ncorr_res, metric = "roc_auc", n = 3) %>% 
  kable()
```
...
```{r}
show_best(rbf_ncoryj_res, metric = "roc_auc", n = 3) %>% 
  kable()
```
...
```{r}
show_best(rbf_ncorq_res, metric = "roc_auc", n = 3) %>% 
  kable()
```
...
```{r}
show_best(rbf_pca_res, metric = "roc_auc", n = 3) %>% 
  kable()
```
...
ggsave(file="manuscript/images/Fig_all_rocs_all_posterior.pdf", 
...
# best models
best_models <- map(top_models, ~select_best(., metric = "roc_auc"))
```
...
lr_final[[2]] %>% 
  extract_fit_engine() %>%
  readr::write_rds(paste0(models_path, "llr_final.rds"))
```
...
gbt_base_final[[2]] %>% 
  extract_fit_engine() %>%
  readr::write_rds(paste0(models_path, "gbt_final.rds"))
```
...
ggsave(file="manuscript/images/Fig_LR_GBT_prob_cutoff.pdf", 
...
Fig_LR_predictors_alt <-
  ...
  scale_y_break(c(3, 28), ticklabels = c(29)) +
  theme(
    legend.position = "top",
    axis.text.x = element_text(
      vjust = 0.9,
...
ggsave(file="manuscript/images/Fig_LR_predictors_alt.pdf", 
...
bt_best_param <- select_best(bt_base_bres, metric = "roc_auc")
...
vip_gbt %>% 
  group_by(variable) %>% 
  summarise(m = median(dropout_loss)) %>% 
  arrange(-m) %>% 
  kable()
```
...
ggsave(file="manuscript/images/Fig_GBT_predictors_alt.pdf", plot=Fig_GBT_predictors_alt, width = 9, height=12, units = "in")
