...
Alternative 2: no legend, better names
...
Fig_LR_predictors_alt2 <-
  ...
  filter(term != "(Intercept)") %>%
  ggplot(aes(reorder(term, odds.ratio, decreasing = F), odds.ratio_)) +
  geom_col(aes(fill = effect)) +
  scale_fill_manual(values = c("#A6CEE3", "#1F78B4"),
                    guide = guide_legend(title = NULL)) +
  theme(
  ...
  xlab("") +
  ylab("odds ratio") +
  # scale_x_discrete(labels = c("TEM beta-lac.", "MPhT", "repeats > 100 nt on plasmids", 
  #                             "IS families", "IS30", "median AR length on plasmids", 
  #                             "median repaet length on chrom.", "CAT", "IS21", 
  #                             "beta-lac. on plasmids", "ISAS1", "median AR")) +
  coord_flip()
