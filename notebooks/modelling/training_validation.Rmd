...
get_models_set <- function(file_path, model_specs_list, recipes_list) {
  if (!file.exists(file_path)) {
    print("File does not exist. Proceeding with building the workflowset of the models.")
    models_set <-
      workflow_set(
...
models_lsvm <-
  get_models_set(
    file_path = "~/Data/HeteroR/results/models/scheme12/models_lsvm.rds",
    model_specs_list = list(linSVM = lsvm),
    recipes_list = list(
      ncorr = ncorr_recipe,
...
```{r}
read_models_svm_mlp <- function(file_path, model_specs_list) {
  ...
  if (!file.exists(file_path)) {
    ...
    saveRDS(object = models_svm_mlp, file = file_path)
  } else {
...
models_svm_mlp <-
  read_models_svm_mlp(
    file_path = "~/Data/HeteroR/results/models/scheme12/models_svm_mlp.rds",
...
```{r, fig.width=6, fig.height=8}
autoplots <- map(models_svm_mlp$result, ~autoplot(.x))

ggpubr::ggarrange(
  plotlist =  autoplots,
  ncol = 2,
  nrow = 2,
  labels = c("", "", "", ""), font.label = list(size=12)
)
```
