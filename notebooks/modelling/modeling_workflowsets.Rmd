...
```{r}
lsvm_best_res <- list(
    models_svm_mlp$result[[1]],
    models_svm_mlp$result[[6]],
    models_svm_mlp$result[[11]],
    models_svm_mlp$result[[16]]
  )
...
```{r}
psvm_best_res <- list(
    models_svm_mlp$result[[2]],
    models_svm_mlp$result[[7]],
    models_svm_mlp$result[[12]],
    models_svm_mlp$result[[17]]
  )
...
```{r}
rbfsvm_best_res <- list(
    models_svm_mlp$result[[3]],
    models_svm_mlp$result[[8]],
    models_svm_mlp$result[[13]],
    models_svm_mlp$result[[18]]
  )
...
```{r}
mlp_nnet_best_res <- list(
    models_svm_mlp$result[[4]],
    models_svm_mlp$result[[9]],
    models_svm_mlp$result[[14]],
    models_svm_mlp$result[[19]]
  )
...
```{r}
bag_mlp_best_res <- list(
    models_svm_mlp$result[[5]],
    models_svm_mlp$result[[10]],
    models_svm_mlp$result[[15]],
    models_svm_mlp$result[[20]]
  )
...
```{r}
knn_best_res <- list(
    models_mars_knn$result[[2]],
    models_mars_knn$result[[5]],
    models_mars_knn$result[[8]],
    models_mars_knn$result[[11]]
  )
...
```{r}
mars_best_res <- list(
    models_mars_knn$result[[1]],
    models_mars_knn$result[[3]],
    models_mars_knn$result[[4]],
    models_mars_knn$result[[6]],
    models_mars_knn$result[[7]],
    models_mars_knn$result[[9]],
    models_mars_knn$result[[10]],
    models_mars_knn$result[[12]]
  )
...
```{r}
best_models_res<- list(models_llr$result[[2]], # baseyj_LLR
                   models_xgb_rf_light$result[[2]], # base_bt_light 
                   models_svm_mlp$result[[15]], # ncorrorq_bag_mlp
                   models_xgb_rf_light$result[[1]], ## base_rf
                   models_svm_mlp$result[[6]], # ncorryj_lsvm  
                   models_svm_mlp$result[[12]] # ncorrorq_psvm 
...
best_models_rocs <-
  map2_dfr(best_models_res,
  best_models_names,
  function(x, y)
    make_roc(x, y))

Fig_7A <- best_models_rocs %>% 
  ggplot(aes(x = 1 - specificity, y = sensitivity, col = model)) + 
  geom_path(lwd = 0.5, alpha = 0.8) +
  geom_abline(lty = 3) + 
  coord_equal() + 
  #scale_color_viridis_d(direction = 1, option = "B")
  scale_color_brewer(palette="Set1") +
  ggtitle("The best models, ROC curves")
Fig_7A
```
...
```{r}
mod_comparison <- res_comp_table(best_models_res,
                                 best_models_names)
...
```{r}
Fig_7AB <- ggarrange(Fig_7A, Fig_7B,
                    labels = c("A", "B"),
...
yj_recipe_tuned_lr <-
  update_corr_treshold(step_no = 6, corr_threshold = best_perf_hyp$corr_tune[1])
...
```{r}
Fig_7C <- senspec_plot(lr_final[[2]], "LLR")+
...
Fig_7D <- lr_coeff_df %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.9, hjust=0.9, size = 12), legend.position = "bottom") +
  xlab("") +
  coord_flip()
Fig_7D
```
...
```{r}
Fig_7CD <- ggarrange(Fig_7C, Fig_7D,
                    labels = c("C", "D"),
                    ncol = 2, nrow = 1)
Fig_7CD
```
