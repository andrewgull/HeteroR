...
```{r}
make_roc(lr_ncorr_res, "LR (no corr)") %>% autoplot()
```
...
lr_ncorq_res %>% show_best("roc_auc", n = 5)
```

```{r}
make_roc(lr_ncorq_res, "LR (orq)") %>% autoplot()
```
...
## BT with NCORR + ORQ recipe

```{r, fig.width=14, fig.height=8}
xgb_ncorq_bres <- readRDS(paste0(models_path, "btb_ncorr_orq.rds"))
...
```{r}
mlp_ncorq_kbres <- readRDS(paste0("/Volumes/MyFolder$/andgu885/HeteroR/models/", "mlpb_keras_ncorr_orq.rds"))
...
```{r, fig.width=8, fig.height=6}
roc_selected_df <- map2_dfr(list(lr_ncorr_res, xgb_ncorr_bres, xgb_ncorq_bres, nnet_ncorr_res),
                            c("LR no corr", "BT no corr", "BT no corr + ORQ", "MLP no corr + ORQ nnet"), 
...
roc_selected_df %>% 
  ...
  theme_bw()
```
So, BT + no corr + ORQ still looks better than BT with the old no corr version (without ORQ)
