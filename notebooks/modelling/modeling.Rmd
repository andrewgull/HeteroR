...
```{r, fig.width=10}
lr_ncorr07_res <- readRDS(paste0(models_path, "lr_ncorr_rs20_corr07.rds"))

autoplot(lr_ncorr07_res)
```
...
```{r, fig.width=8, fig.height=6}
roc_selected_df <- map2_dfr(list(lr_ncorr_res, xgb_ncorr_bres, xgb_ncorr_bres_long, xgb_main_bres, nnet_ncorr_bres),
                            c("LR", "XGB no corr", "XGB long", "XGB main", "MLP b"), 
                            function(x, y) make_roc(x, y))
...
xgb_wf <- workflow() %>% 
  add_model(xgb_mod) %>% 
  add_recipe(ncorq_recipe) # the recipe here should be the one that was used for this model
