...
**NB:** As comparison in EDA notebook showed, OQ-normalization works better than Yeo-Johnson transformation.
...
If you want to use UMAP instead of real features, then you should not use supervised UMAP.

Also, (supervised) UMAP recipe dumps core for whatever reason

## NCORR recipe

For LR I used space filling grid, cause Bayesian would cause errors during resampling and the advantage it gives is marginal.
...
> "Error in glmnet::glmnet(x = maybe_matrix(x), y = y, alpha = ~1, family = \"binomial\"): x should be a matrix with 2 or more columns"
...
```{r}
svm_ncorr_res <- readRDS(paste0(models_path, "svm_ncorr.rds"))
```
...
> The Gaussian process model is being fit using 12 features but only has 9 data points to do so. This may cause errors or a poor model fit
...
# calculate rocs
models_list <- list(knn_ncor_bres, lr_ncorr_res, lr_pca_res, mars_ncorr_res, mars_pca_res, rf_main_bres, xgb_main_bres, svm_ncorr_res, svm_poly_res)
models_names <- c("KNN (no corr)", "LR (no corr)", "LR (PCA)", "MARS (no corr)", "MARS (PCA)", "RF (all)", "XGB (all)", "SVM linear (no corr)", "SVM polynomial (no corr)")
...
XGB here looks better than in previous version (it reaches sensitivity = 1 faster)
...
Sequencing related features (coverage and read length) are here - among the most important
...
Previous both CM and performance looked a bit better. 
...
sens_spec_j_plot <- ggplot(threshold_data, aes(x = .threshold, y = .estimate, color = .metric, alpha = group)) +
  ...
  labs(
    x = "Probability",
    y = "Metric Estimate",
    title = "Final model: Boosting Trees"
  )
...
Using suggested j-index value, I can optimize probability cut-off in order to have 100% sensitivity of the model
...
## Probability cutoff adjustment by 2nd maximum j-index

j-index a bit lower but spec is higher

```{r, fig.width=6}
second_max_j_index_threshold <- threshold_data %>%
  filter(.metric == "j_index") %>%
  filter(.estimate != max(.estimate)) %>%
  filter(.estimate == max(.estimate)) %>% 
  pull(.threshold)

pred_optimized_2 <- final_fit %>%
  collect_predictions() %>% 
  mutate(
    .pred = make_two_class_pred(
      estimate = .pred_HR, 
      levels = levels(resistance), 
      threshold = second_max_j_index_threshold
    )
  ) %>%
  select(resistance, contains(".pred"))

cm_optimized_2 <- pred_optimized %>% 
  conf_mat(truth = resistance, estimate = .pred)

autoplot(cm_optimized_2, type = "heatmap")
```

```{r}
summary(cm_optimized_2)
```
...
```{r}
# c("KNN (no corr)", "LR (no corr)", "LR (PCA)", "MARS (no corr)", "MARS (PCA)", "RF (all)", "XGB (all)", "SVM linear (no corr)", "SVM polynomial (no corr)")
...
test_stack_model_1 <- 
  test_stack_1 %>% 
  blend_predictions(penalty = seq(0.01, 0.5, 0.05))

autoplot(test_stack_model_1)
```

```{r}
autoplot(test_stack_model_1, type = "members")
```

```{r}
autoplot(test_stack_model_1, type = "weights")
```
...
df_stack_test_1 <- 
  df_test %>%
  bind_cols(predict(test_stack_model_1, .))
...
autoplot(test_stack_model_2)
```

```{r}
autoplot(test_stack_model_2, type = "members")
```

```{r}
autoplot(test_stack_model_2, type = "weights")
```
