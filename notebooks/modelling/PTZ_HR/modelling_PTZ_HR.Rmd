...
feat_num <- select(feat, -c(resistance, n.beta.lac.3, n.beta.lac.3))
...
feat_nocorr$n.beta.lac.4 <- feat$n.beta.lac.4
...
fit_rf <- train(resistance ~ ., 
                 ...
                 method = "rf", 
                 trControl = fit_ctrl_roc,
                 tuneGrid = expand.grid(.mtry = seq(2, 15, 1)),
                 ntree = 50,
                 nodesize = 1,
                 verbose = FALSE)
...
# plot
plot(perf_rf, main = "Random Forest ROC curve", col = "steelblue", lwd = 3)
abline(a = 0, b = 1, lwd = 3, lty = 2, col = 1)
...
legend(x = 0.65,y = 0.95, c("TPR (Recall)", "FPR (1-Spec)"), 
       lty = 1, col =c('steelblue', 'red'), bty = 'n', cex = 1, lwd = 2)
...
plot(imp_vars_rf, main="Variable Importance with Random Forest")
```
