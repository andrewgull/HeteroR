...
```{r}
# extended grid search preparation
xgb_param <- extract_parameter_set_dials(xgb_mod)
xgb_param
xgb_param %>% extract_parameter_dials("tree_depth")

# one way to make a regular grid
grid_regular(xgb_param, levels = 2)

# or more flexible
xgb_param %>% 
  grid_regular(levels = c( min_n = 2, tree_depth = 2, learn_rate = 2, loss_reduction = 2, sample_size = 2, stop_iter = 2))

# or random grid of defined size
xgb_param %>% grid_random(size = 10)
```
