...
```{r, cache=F}
feat_amp <- read_csv("data/features_amp_strain.csv", na = c("NA", "-Inf"))
...
feat_amp <- feat_amp %>% select(-c(strain))
...
`APH4` contains only 0, I wull remove the whole column
...
feat_amp_scaled <- as_tibble(
  scale(
    feat_amp %>% select(-c(resistance, n.beta.lac.3))
    ))
...
```{r, fig.width=4, fig.height=4, cache=F}
...
x_nocorr <- as.matrix(training_nocorr %>% select(-resistance))  # we don't needs 'resistance' here
...
# to avoid slowness of caret
x_all <- as.matrix(training_all %>% select(-resistance))  # we don't needs 'resistance' here
...
```{r}
pred_best <- c("resistance", "n.beta.lac.3", "ampC", "med.dist.oriC", "n.genes.plus.strand", "n.genes.plasmids", "med.AR.len.cen", "max.rep.len", "n.rep.500", "med.rep.len.100")
```
...
feat_selected <- feat_amp_scaled %>% 
  select(c(resistance, n.beta.lac.3, ampC, n.plasmids, med.rep.len, med.tot.rep.len, max.rep.len, n.rep.total, med.AR.len.cen, med.dist.oriC, n.genes.plasmids))
...
# to avoid slowness of caret
x_hand <- as.matrix(training_hand %>% select(-resistance))
...
```{r}
LR_coeff <- as_tibble(fit_LR_nocorr_roc$finalModel$W) %>% gather(key = "predictor", value = "log.odds", 1:27) %>% mutate(odds.ratio = exp(log.odds))

LR_coeff
```

A coefficient here is the change in log-odds of the response as a result of a unit change in the predictor variable

```{r}
#LR_coeff_td <- gather(LR_coeff, "type", "value", 2:3)
library(ggpubr)

feat_imp_rlr_or <- ggplot(LR_coeff, aes(reorder(predictor, abs(log.odds)), odds.ratio))+
  geom_col(fill="steelblue")+
  coord_flip()+
  xlab("")+
  ylab("odds ratio")

feat_imp_rlr_lo <- ggplot(LR_coeff, aes(reorder(predictor, abs(log.odds)), log.odds))+
  geom_col(fill="steelblue")+
  coord_flip()+
  xlab("")+
  ylab("log odds")
...
feat_imp_rlr <- annotate_figure(
  ggarrange(feat_imp_rlr_or, feat_imp_rlr_lo, ncol = 2), 
  top = text_grob("Binomial Logistic Regression coefficients"))

#annotate_figure(feat_imp_rlr, top = text_grob("Binomial Logistic Regression coefficients"))

feat_imp_rlr
```

Odds ratio 420 means: for a one unit increase in `n.beta.lac`, the odds of a strain to be R increases by a factor of 420 (huge increase)

Odds ration 0.015 means: for one unit increase in `n.ampC`, the odds of a strain to be R increases by a factor of 0.015 (huge decrease)
...
feat_imp_rf <- plot(imp_vars_rf_nocorr, main="Variable Importance with RF (non-correlated)")
...
feat_imp_xgb_all <- plot(imp_vars_xgb_all, main="Variable Importance with XGB (all)")
...
feat_imp_xgb_nocorr <- plot(imp_vars_xgb_nocorr, main="Variable Importance with XGB (nocorr)")
...
feat_imp_xgb_best <- plot(imp_vars_xgb_best, main="Variable Importance with XGB (best)")
...
# write to file
write.csv(perf_metrics, "data/metrics.csv", row.names = FALSE)
