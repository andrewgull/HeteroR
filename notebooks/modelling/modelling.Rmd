...
roc_df <- map2_dfr(list(lr_ncorr_res, xgb_ncorr_bres),
                            c("LR",  "XGB ncorr"), 
                            function(x, y) make_roc(x, y))
...
mod_comparison <- map2(list(lr_ncorr_res, xgb_main_bres, xgb_ncorr_bres, nnet_ncorr),
       c("LR", "XGB all", "XGB ncorr", "MLP ncorr"),
       ~ best_aucs(res_obj = .x, model_name = .y)) %>% # model_names is not passed correctly
  reduce(inner_join, by = c("id", "id2")) %>% 
  set_names(c("id", "id2", "LR", "XGB.all", "XGB.ncorr", "MLP.ncorr")) %>% 
  unite(id, id, id2)
