...
```{r}
mlp_ncoryj <- readRDS(paste0(models_path, "mlp_ncoryj_p20_test.rds"))
...
roc3_df <-
  map2_dfr(top_models,
           c("LR", "pSVM", "lSVM", "MLP", "XGB", "XGB rfe"),
           function(x, y)
             make_roc(x, y))
roc3_df %>% 
  ggplot(aes(x = 1 - specificity, y = sensitivity, col = model)) + 
...
run_last_fit("XGB.ncorr", xgb_spec_blank, xgb_ncorr_spec_tuned, recipe = ncorr_recipe_tuned_xgb)
```
