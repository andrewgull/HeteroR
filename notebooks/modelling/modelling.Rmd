...
```{r}
make_roc(lsvm_ncoryj_res, "") %>% 
  autoplot() + ggtitle("linSVM (no corr. + YJ)")
```
...
psvm_ncorr_res <- readRDS(paste0(models_path, ""))
...
```{r}
xgb_main_bres <- readRDS(paste0(models_path, "btb_main_rs20_new.rds"))
...
```{r}
# corr tuned; bayes search
nnet_ncorr <- readRDS(paste0(models_path, "mlp_ncorr_rs20_new.rds"))
...
```{r}
# corr tune
nnet_ncorq <- readRDS(paste0(models_path, "mlp_ncorq_rs20_new.rds"))
...
```{r}
make_roc(mlp_ncoryj, "") %>% 
  autoplot() + ggtitle("MLP (no corr. + YJ)")
```
...
```{r}
mlp_comp <-
  map2(list(mlp_ncoryj, mlp_pca),
       c("ncoryj", "pca"),
       ~ best_aucs(res_obj = .x, model_name = .y)) %>% 
  reduce(inner_join, by = c("id", "id2")) %>% 
  set_names("id", "id2", "ncoryj", "pca") %>% 
  unite(id, id, id2)
...
```{r}
roc_df <- map2_dfr(list(lr_ncorr_res_new, mlp_ncoryj, xgb, rf_ncorr_res),
                            c("LR", "MLP", "XGB", "RF"), 
...
```{r}
model_names <- c("LR",  "RF", "MLP PCA", "MLP", "XGB", "RF")

poster_comp <-
  map2(list(lr_ncorr_res_new, rf, mlp_pca, mlp, xgb, rf_ncorr_res),
       model_names,
       ~ best_aucs(res_obj = .x, model_name = .y)) %>% # model_names is not passed correctly
  reduce(inner_join, by = c("id", "id2")) %>% 
  set_names(c("id", "id2", model_names)) %>% 
  unite(id, id, id2)

df_posterior <- perf_mod(poster_comp, iter = 2000, seed = 100, refresh = 0, chains = 5, cores = 2)

autoplot(df_posterior)
```

```{r}
preproc_diff <- contrast_models(df_posterior, seed = 100) 
...
# roc_df <- map2_dfr(list(lr_ncorr_res, mars_pca_res, lsvm_ncorr_res, psvm_pca_res, knn_ncorq_res, rf_main_res, xgb_main_bres, mlp_ncorq_bres),
#                             c("LR", "MARS", "linSVM", "polySVM", "KNN", "RF", "XGB", "MLP"), 
#                             function(x, y) make_roc(x, y))

roc_df <- map2_dfr(list(lr_ncorr_res, xgb_ncorr_bres, rf_ncorr_res, nnet_ncorq),
                            c("LR ncorr",  "XGB ncorr", "RF ncorr", "MLP ncorq"), 
                            function(x, y) make_roc(x, y))
...
XGB's ROC looks better than others
...
```{r}
# mod_comparison <- map2(list(lr_ncorr_res, mars_pca_res, lsvm_ncorr_res, psvm_pca_res, knn_ncorq_res, rf_main_res, xgb_main_bres, mlp_ncorq_bres),
#        c("LR", "MARS", "linSVM", "polySVM", "KNN", "RF", "XGB", "MLP"),
#        ~ best_aucs(res_obj = .x, model_name = .y)) %>% # model_names is not passed correctly
#   reduce(inner_join, by = c("id", "id2")) %>% 
#   rename("LR" = .y.x, "MARS" = .y.y, "linSVM" = .y.x.x, "polySVM" = .y.y.y, "KNN" = .y.x.x.x, "RF" = .y.y.y.y, "XGB" = .y.x.x.x.x, "MLP" = .y.y.y.y.y) %>% 
#   unite(id, id, id2)

mod_comparison <- map2(list(lr_ncorr_res, xgb_ncorr_bres, rf_ncorr_res, nnet_ncorq),
       c("LR ncorr",  "XGB ncorr", "RF ncorr", "MLP ncorq"),
       ~ best_aucs(res_obj = .x, model_name = .y)) %>% # model_names is not passed correctly
  reduce(inner_join, by = c("id", "id2")) %>% 
  set_names(c("id", "id2", "LR", "XGB", "RF", "MLP")) %>% 
  unite(id, id, id2)

mod_posterior <- perf_mod(mod_comparison, iter = 2000, seed = 100, refresh = 0, chains = 5, cores = 8)

autoplot(mod_posterior) + scale_color_brewer(palette = "Set1")
```

LR, XGB and MLP stand out
...
```{r, fig.width=10}
contrast_models(mod_posterior, seed = 100) %>% autoplot()
```
