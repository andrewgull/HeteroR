...
```{r}
lsvm_ncorr_res <- readRDS(paste0(models_path, "lsvm_ncorr_rs20_corr_tune_wl.rds"))
...
```{r, fig.width=10}
psvm_ncorr_res <- readRDS(paste0(models_path, "psvm_ncorr_rs20_corr_tune_wl.rds"))
...
Space filling grid proved to be less efficient in finding
hyper-parameters than Bayesian optimization
...
#xgb_ncorr_bres <- readRDS(paste0(models_path, "btb_ncorr_rs50_corr_tune.rds"))
...
### Heatmap of the best iteration

```{r}
xgb_ncorr_bres %>% 
  collect_predictions() %>% 
  filter(.iter == 21) %>% 
  mutate(.pred_HRf = factor(if_else(.pred_HR > 0.5, "HR", "nonHR"))) %>% 
  conf_mat(truth = resistance, estimate = .pred_HRf) %>% 
...
```{r}
make_roc(xgb_ncorr_wl, "XGB (no corr)") %>% 
  autoplot()
```
...
```{r, fig.width=14}
xgb_ncorr_bres_long <- readRDS(paste0(models_path, "btb_ncorr_rs20_k5.rds"))
...
I also tried 100 iterations with 8 starting points, it was not better
than 50 iterations and 8 starting points, but the performance plot looks
peculiar:
...
```{r, fig.width=14, fig.height=8}
xgb_pca_bres <- readRDS(paste0(models_path, "btb_pca_rs50_wl.rds"))

xgb_pca_bres %>% plot_race()
```
...
```{r, fig.width=12}
nnet_ncorr <- readRDS(paste0(models_path, "mlp_ncorr_rs20_corr_tune_wl.rds"))

nnet_ncorr %>% plot_race()
```
...
```{r}
nnet_ncorr %>% show_best("roc_auc", n = 5)
```
...
```{r, fig.width=10}
nnet_ncorq <- readRDS(paste0(models_path, "mlp_ncorq_rs20_corr_tune_wl.rds"))

nnet_ncorq %>% plot_race()
```
...
```{r}
make_roc(nnet_ncorq, "MLP") %>% autoplot()
```
...
mod_comparison <- inner_join(
  lr_ncorr_res %>% 
    ...
    select(id, LR_norm = .estimate),
  lr_ncorq_res %>% 
    collect_metrics(summarize =FALSE) %>% 
    filter(.metric == "roc_auc") %>% 
    select(id, LR_orq = .estimate)
)
