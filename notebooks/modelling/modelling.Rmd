...
```{r, fig.width=6, fig.height=3}
lr_main_LOO <- readRDS(paste0(models_path, "lr_main_space_p30_LOO.rds"))
...
lr_rocs <-
  ...
  c("lasso.ncorr",  "lasso.ncorr.yj", "lasso.pca.yj", "ridge.ncorr.yj", "elnet.ncorr.yj"),
  function(x, y)
    make_roc(x, y))
...
rf_comp <-
  res_comp_table(
    res_list = list(rf_main_res, rf_mainrfe_res, rf_ncorr_res, rf_ncoryj_res),
...
## RFE + main recipe + race

```{r}
xgb_mainrfe_race <- readRDS(paste0(models_path, "bt_mainrfe_res_p30.rds"))
...
## RFE + main srcipe + Bayesian search
...
```{r}
mlp_yjrfe_race <- readRDS(paste0(models_path, "mlp_yjrfe_res_p30.rds"))
...
```{r}
mlp_bag_yjrfe_res <- readRDS(paste0(models_path, "bag_mlp_yjrfe_res_p30.rds"))
...
-   XGB: ncorr bayes or main rfe; the rest are very close to each other
...
roc3_df <-
  map2_dfr(top_models,
           c("LR", "pSVM", "lSVM", "MLP", "XGB ncorr", "XGB rfe"),
           function(x, y)
             make_roc(x, y))
roc3_df %>% 
  ggplot(aes(x = 1 - specificity, y = sensitivity, col = model)) + 
...
```{r}
# XGB rfe specs /number 6/
xgb_rfe_spec_tuned <- boost_tree(
  trees = 50,
...
# XGB rfe last fit and metrics
xgb_rfe_final <- run_last_fit(
  "XGB.rfe",
  xgb_spec_blank,
  xgb_rfe_spec_tuned,
  recipe = main_rfe_recipe,
  return_fit = TRUE
)
...
rocs_final <- map2(list(xgb_ncorr_final[[2]], lr_final[[2]]), mod_labels, ~make_final_roc(.x, .y))
...
XGB reaches higher sensitivity faster than LR on the same level of sensitivity
...
Around 40 predictors were removed during `step_nzv()` and about 10 during `step_corr()` or 80 during LASSO
