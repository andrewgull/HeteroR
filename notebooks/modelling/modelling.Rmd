...
final_fit %>% 
  extract_fit_parsnip() %>% 
  vip(num_features = 50, include_type = T) 
```
...
shap <- fastshap::explain(final_xgb_mod$fit, X = X, exact = TRUE)
...
```{r}
feat <- prep(main_recipe, df_train) %>% 
  juice()
...
```{r}
lr_fit <- last_wf %>% fit(df_train) %>% extract_fit_parsnip()
```
...
prediction_all_data <- predict(lr_fit, new_data=data_strain_proc, type="prob") %>% 
  select(.pred_HR) %>% 
  mutate(.pred_class = if_else(.pred_HR >= max_j_index_threshold, "HR", "nonHR"),
