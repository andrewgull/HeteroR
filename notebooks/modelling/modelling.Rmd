...
models_path_old <- "~/Data/HeteroR/results/models/scheme12_old/"
models_path_cv5 <- "~/Data/HeteroR/results/models/scheme123/folds5/"
```
...
```{r}
lr_rocs <- map2_dfr(list(lr_ncorr_res, lr_ncoryj_res, lr_pcayj_res),
                            c("LR ncorr",  "LR ncorr+yj", "LR pca+yj"), 
...
```{r}
lr_comp <- res_comp_table(res_list = list(lr_ncorr_res, lr_ncoryj_res, lr_pcayj_res), 
                          mod_names = c("ncorr", "ncorr.yj", "pca.yj"))

lr_posterior <- perf_mod(lr_comp, iter = 2000, seed = 100, refresh = 0, chains = 5, cores = 2)
...
```{r}
# tuned recipe
best_xgb <- xgb_ncoryj_res %>% 
  select_best("roc_auc")

# recipe with tuned corr
yj_recipe_best_corr <- 
  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>% 
  ...
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% 
  step_corr(threshold = best_xgb$corr_tune)
...
xgb_wf <- workflow() %>% 
  add_model(xgb_spec) %>% 
  add_recipe(yj_recipe_best_corr) # the recipe here should be the one that was used for this model
...
# tuned recipe
best <- lr_ncoryj_res %>% 
  # show_best("roc_auc") %>% filter(.config == "Preprocessor07_Model1") # lower corr. threshold
...
# another way
#best <- select_by_pct_loss(mlp_ncoryj, "corr_tune")
  
# the 2nd best
#best <- filter(show_best(mlp_ncoryj, "roc_auc"), .config == "Preprocessor18_Model1")
...
pcayj_recipe_tuned <- 
  ...
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% 
  step_pca(all_predictors(), num_comp = best$num_comp) %>% 
  step_normalize(all_numeric_predictors())
...
mlp_wf <- workflow() %>% 
  ...
  add_recipe(pcayj_recipe_tuned) # the recipe here should be the one that was used for this model
...
```{r}
#best <- select_best(mlp_ncoryj, "roc_auc")

# let's choose model with the highest corr. threshold
best <- mlp_ncoryj %>% 
  show_best("roc_auc") %>% 
  filter(.config == "Preprocessor15_Model1")
...
## How does the PCA result look?

If your winning model had PCA as input, look at the PCA results here
...
# the recipe here is without SMOTE step
pcayj_recipe_tuned %>% 
  prep() %>% 
  juice() %>% 
  plot_validation_results(c("PC1", "PC2", "PC3")) + 
  ggtitle("PCA input for MLP")
```
...
# predictors actually left in the model
yj_data_proc$template %>% names()
```
...
final_fit %>% 
  extract_fit_parsnip() %>% 
  vip(num_features = 60, include_type = T) 
```
...
```{r}
# find the step number corresponding to PCA, here it's 6
data_pca <- pcayj_recipe_tuned %>% prep(retain=TRUE)
...
data_pca %>%
  plot_top_loadings(component_number <= 3, n = 10) + 
  scale_fill_brewer(palette = "Paired") +
  ggtitle("PCA top loadings")
```
