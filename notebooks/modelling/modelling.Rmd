...
#lr_base_res <- readRDS(paste0(models_path, "lr_base_space_p30.rds"))
...
#lr_base_yj_res <- readRDS(paste0(models_path, "lr_baseyj_space_p30.rds"))
...
```{r lr orq}
#lr_base_orq_res <- readRDS(paste0(models_path, "lr_ncorq_p30.rds"))
...
```{r}
# workflow ids in this file
readRDS(paste0(models_path, "models_mars_knn.rds")) %>% pull(wflow_id)
```
...
```{r}
mars_ncorq_res <- read_wfset("models_mars_knn.rds", "base_orq_mars")
...
```{r}
mars_bag_ncorq_res <- read_wfset("models_mars_knn.rds", "base_orq_bag_mars")
...
gbt_base_bres <- readRDS(paste0(models_path, "base_bt_bres_light.rds"))

autoplot(gbt_base_bres)
```

### Best models

```{r}
gbt_base_bres %>% show_best(metric="roc_auc")
```
...
bind_rows(get_metrics(gbt_base_final[[2]], "GBT base"),
...
```{r}
senspec_plot(gbt_base_final[[2]], "GBT base")
```
...
cm_optimized_xgb <- optimize_prediction(gbt_base_final[[2]]) %>% 
  ...
  autoplot(type = "heatmap") + ggtitle("GBT base")
...
bind_rows(
  get_metrics2(gbt_base_final[[2]], "GBT base"),
...
lr_coeff_df %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.9, hjust=0.9, size = 12)) +
  xlab("") +
  coord_flip()
```
