...
cor.matrix <- cor(feat_amp[,c(2:39)], use = "pairwise.complete.obs")
corrplot::corrplot(cor.matrix, type="upper", tl.col = "black")
```

```{r, fig.width=12, fig.height=12}
corrplot::corrplot(cor.matrix, type="upper", method = "number", number.cex=0.8, number.digits=1, tl.col = "black")
```
...
feat_amp_proc <- select(feat_amp, -c(resistance, n.beta.lac.3))
...
x_nocorr <- training_nocorr[,-26]
y_nocorr <- training_nocorr[,26]
...
# to avoid slowness of caret
x_all <- training_all[,-1]
y_all <- training_all[,1]
...
```{r}
cluster <- makeCluster(detectCores() - 2) # leave two cores
...
x_best <- training_best[,-1]
y_best <- training_best[,1]
```
...
# to avoid slowness of caret
x_hand <- training_hand[,-1]
y_hand <- training_hand[,1]
...
Train control here has less repeats and `expand.grid()` function is used
...
fit_RF <- train(x_all, y_all$resistance,  
                 method = "rf", 
...
fit_RF_best <- train(x_best, y_best$resistance, 
                 method = "rf", 
                 trControl = fit_ctrl_rf_roc,
                 #tuneGrid=tunegrid,
                 verbose = FALSE,
...
fit_XGB <- train(x_all, y_all$resistance, 
                 method = "xgbDART", 
...
fit_XGB_best <- train(x_best, y_best$resistance, 
                 method = "xgbDART", 
                 trControl = fit_ctrl_xgb_roc,
                 verbose = FALSE,
                 metric="ROC",
