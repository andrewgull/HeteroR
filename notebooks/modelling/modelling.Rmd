...
```{r}
# correlation threshold was tuned as well
lr_ncorr_res_new <- readRDS(paste0(models_path, "lr_ncorr_p20.rds"))

show_best(lr_ncorr_res_new)
```
...
```{r}
# the corr is tuned
rf_ncorr_res <- readRDS(paste0(models_path_old, "rf_ncorr_rs20_new.rds"))
...
```{r}
xgb_ncorr_bres <- readRDS(paste0(models_path, "btb_ncorr_rs20_new.rds"))
...
```{r}
mlp_comp <-
  map2(list(nnet_ncorr, nnet_ncorq, nnet_pca, mlp_ncorq_bres),
       c("norm", "orq", "pca", "bayes"),
       ~ best_aucs(res_obj = .x, model_name = .y)) %>% # model_names is not passed correctly
  reduce(inner_join, by = c("id", "id2")) %>% 
  rename("norm" = .y.x, "orq" = .y.y, "pca" = .y.x.x, "bayes" = .y.y.y) %>% 
  unite(id, id, id2)
...
ORQ-normalization + Bayesian grid search outperform other preprocessors
...
```{r}
# tuned recipe
best_corr <- nnet_ncorq %>% 
  select_best("roc_auc") %>% 
  pull(corr_tune)

# recipe with tuned corr
ncorq_recipe_tuned <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>% 
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(threshold = best_corr) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)
...
mlp_wf <- workflow() %>% 
  add_model(mlp_mod) %>% 
  add_recipe(ncorq_recipe_tuned) # the recipe here should be the one that was used for this model
