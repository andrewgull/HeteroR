...
## Comparison of the preprocessing in MARS

### ROCs overlapped

```{r}
mars_rocs <- map2_dfr(list(mars_ncorr_res, mars_ncoryj_res,mars_yjmainrfe_race, bagmars_yjmainrfe_race), c("ncorr sfg", "ncorr.yj sfg","mainrfe.yj race","bag mainrfe.yj race"), 
                            function(x, y) make_roc(x, y))
...
```{r}
mars_comp <- res_comp_table(res_list = list(mars_ncorr_res, mars_ncoryj_res,mars_yjmainrfe_race, bagmars_yjmainrfe_race),
                            mod_names = c("ncorr sfg", "ncorr.yj sfg","mainrfe.yj race","bag mainrfe.yj race"))
...
## lsvm with Recursive Features Elimination + YJ recipe
...
```{r}
make_roc(lsvm_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("lsvm (rfe + YJ)")
```

## lsvm with Recursive Features Elimination + YJ recipe Bayesian grid tuning
...
```{r}
make_roc(lsvm_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("lsvm (rfe + YJ) bayesian grid")
```
...
## psvm with Recursive Features Elimination + YJ recipe
...
```{r}
make_roc(psvm_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("psvm (rfe + YJ)")
```

## psvm with Recursive Features Elimination + YJ recipe Bayesian grid 
...
```{r}
make_roc(psvm_yjmainrfe_bayes, "") %>% 
  autoplot() + ggtitle("psvm (rfe + YJ) Bayesian grid search")
```
...
## rbfsvm with Recursive Features Elimination + YJ recipe
...
```{r}
make_roc(rbfsvm_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("rbfsvm (rfe + YJ)")
```

## rbfsvm with Recursive Features Elimination + YJ recipe Bayesian grid
...
```{r}
make_roc(rbfsvm_yjmainrfe_bayes, "") %>% 
  autoplot() + ggtitle("rbfsvm (rfe + YJ) Bayesian grid")
```
## Comparison of the preprocessing in SVM

### ROCs overlapped

```{r}
svr_rfe_rocs <- map2_dfr(list(lsvm_yjmainrfe_race, psvm_yjmainrfe_race, rbfsvm_yjmainrfe_race, lsvm_yjmainrfe_bayes, psvm_yjmainrfe_bayes, rbfsvm_yjmainrfe_bayes), c("LSVM YJ rfe race",  "PSVM YJ rfe race", "RBFSVM YJ rfe race","LSVM YJ rfe bayes",  "PSVM YJ rfe bayes", "RBFSVM YJ rfe bayes" ), 
                            function(x, y) make_roc(x, y))
...
```{r}
svm_rfe_comp <-  res_comp_table(res_list = list(lsvm_yjmainrfe_race, psvm_yjmainrfe_race, rbfsvm_yjmainrfe_race, lsvm_yjmainrfe_bayes, psvm_yjmainrfe_bayes, rbfsvm_yjmainrfe_bayes),
    mod_names = c("LSVM YJ rfe race",  "PSVM YJ rfe race", "RBFSVM YJ rfe race","LSVM YJ rfe bayes",  "PSVM YJ rfe bayes", "RBFSVM YJ rfe bayes") )
...
autoplot(svm_rfe_posterior)
```

```{r}
svm_rfe_diff <- contrast_models(svm_rfe_posterior, seed = 100) 

summary(svm_rfe_diff)
```
...
## Comparison of the preprocessing in KNN

### ROCs overlapped

```{r}
knn_rocs <- map2_dfr(list(knn_ncorr_res, knn_ncoryj_res, knn_ncorq_res, knn_yjmainrfe_race), c("ncorr", "ncorr.yj", "ncorr.orq","rfe_race.yj"), 
                            function(x, y) make_roc(x, y))
...
```{r, fig.width=5, fig.height=2}
knn_comp <- res_comp_table(res_list = list(knn_ncorr_res, knn_ncoryj_res, knn_ncorq_res, knn_yjmainrfe_race),
                           c("ncorr", "ncorr.yj", "ncorr.orq","rfe_race.yj"))
...
## knn with Recursive Features Elimination + YJ recipe
...
```{r}
make_roc(knn_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("knn (rfe + YJ)")
```

# Naive Bayes, kind of not working.

```{r,eval =FALSE}
nb_ncorr <- readRDS(paste0(models_path, "nb_ncorr_space_p30.rds"))
...
## Random Forest with Recursive Features Elimination

```{r}
rf_mainrfe_race <- readRDS(paste0(models_path, "rf_mainrfe_race_p30.rds"))

autoplot(rf_mainrfe_race)
```
...
```{r}
make_roc(rf_mainrfe_race, "") %>% 
  autoplot() + ggtitle("rf (rfe)")
```
...
## Boosted trees with Recursive Features Elimination

```{r}
bt_mainrfe_race <- readRDS(paste0(models_path, "bt_mainrfe_race_p30.rds"))

plot_race(bt_mainrfe_race)
```

### The best ones

```{r}
show_best(bt_mainrfe_race)
```

### ROC

```{r}
make_roc(bt_mainrfe_race, "") %>% 
  autoplot() + ggtitle("bt (rfe)")
```

# Boosted trees with Recursive Features Elimination Bayesian grid

```{r}
bt_mainrfe_bayes <- readRDS(paste0(models_path, "bt_mainrfe_bayes_p30.rds"))

autoplot(bt_mainrfe_bayes)
```

### The best ones

```{r}
show_best(bt_mainrfe_bayes)
```

### ROC

```{r}
make_roc(bt_mainrfe_bayes, "") %>% 
  autoplot() + ggtitle("bt (rfe)")
```

## Comparison of the preprocessing in RF + XGB

### ROCs overlapped

```{r}
rf_bt_rfe_rocs <- map2_dfr(list(rf_main_res, rf_mainrfe_race, rf_ncorr_res, rf_ncoryj_res,bt_mainrfe_race,bt_mainrfe_bayes), c("RF sfg",  "RF rfe race","RF NCORR sfg","RF NCORR +YJ sfg", "BT rfe race", "BT rfe bayes"), 
                            function(x, y) make_roc(x, y))
...
*post hoc* analysis of resampling results generated by models - via Bayesian ANOVA
```{r}
rf_bt_rfe_comp <-  res_comp_table(res_list = list(rf_main_res, rf_mainrfe_race, rf_ncorr_res, rf_ncoryj_res,bt_mainrfe_race,bt_mainrfe_bayes),
    mod_names = c("RF sfg",  "RF rfe race","RF NCORR sfg","RF NCORR +YJ sfg", "BT rfe race", "BT rfe bayes") )
...
autoplot(rf_bt_rfe_posterior)
```

```{r}
rf_bt_rfe_diff <- contrast_models(rf_bt_rfe_posterior, seed = 100) 

summary(rf_bt_rfe_diff)
```
...
## mlp_nnet with Recursive Features Elimination + YJ recipe

```{r}
mlpnnet_yjmainrfe_race <- readRDS(paste0(models_path, "mlpnnet_yjmainrfe_race_p30.rds"))

plot_race(mlpnnet_yjmainrfe_race)
```
### The best ones

```{r}
show_best(mlpnnet_yjmainrfe_race)
```
### ROC

```{r}
make_roc(mlpnnet_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("mlp nnet (rfe + YJ)")
```
...
mlp_rocs <- map2_dfr(list(mlp_ncoryj, mlp_pca, mlp_ncorq, mlpk_pca, bagmlp_yjmainrfe_race, mlpnnet_yjmainrfe_race),
                            c("MLP ncorr+yj",  "MLP pca+yj", "MLP ncorq", "Keras pca+yj", "bagMLP rfe yj race", "nnetMLP rfe yj race"), 
                            function(x, y) make_roc(x, y))
...
```{r}
mlp_comp <- res_comp_table(res_list = list(mlp_ncoryj, mlp_pca, mlp_ncorq, mlpk_pca, bagmlp_yjmainrfe_race, mlpnnet_yjmainrfe_race),
                           mod_names = c("MLP ncorr+yj",  "MLP pca+yj", "MLP ncorq", "Keras pca+yj", "bagMLP rfe yj race", "nnetMLP rfe yj race"))
...
## bag_mlp with Recursive Features Elimination + YJ recipe

```{r}
bagmlp_yjmainrfe_race <- readRDS(paste0(models_path, "bagmlp_yjmainrfe_race_p30.rds"))

plot_race(bagmlp_yjmainrfe_race)
```
### The best ones

```{r}
show_best(bagmlp_yjmainrfe_race)
```
### ROC

```{r}
make_roc(bagmlp_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("bagmlp (rfe + YJ)")
```

#MARS
## mars with Recursive Features Elimination + YJ recipe

```{r}
mars_yjmainrfe_race <- readRDS(paste0(models_path, "mars_yjmainrfe_race_p30.rds"))

plot_race(mars_yjmainrfe_race)
```

### The best ones
```{r}
show_best(mars_yjmainrfe_race)
```

### ROC
```{r}
make_roc(mars_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("mars (rfe + YJ)")
```

## bag_mars with Recursive Features Elimination + YJ recipe

```{r}
bagmars_yjmainrfe_race <- readRDS(paste0(models_path, "bag_mars_yjmainrfe_race_p30.rds"))

plot_race(bagmars_yjmainrfe_race)
```

### The best ones
```{r}
show_best(bagmars_yjmainrfe_race)
```


### ROC
```{r}
make_roc(bagmars_yjmainrfe_race, "") %>% 
  autoplot() + ggtitle("bag mars (rfe + YJ)")
```
