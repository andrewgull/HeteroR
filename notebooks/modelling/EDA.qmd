...
Here I increase the minimal distance to see the local stucture better

```{r, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
## UMAP Supervised

umap_rec_sup <- orq_recipe %>%
  step_umap(all_numeric_predictors(), outcome = "resistance", num_comp = 3, min_dist = 0.6, neighbors = 25)
...
hr_groups <- left_join(hr_testing_12, hr_testing_13, by = "strain") %>% 
  mutate(hr.group = case_when(resistance.x == "HR" & resistance.y == "HR" ~ "g1",
                              resistance.x == "HR" & resistance.y == "nonHR" ~ "g2",
                              resistance.x == "nonHR" & resistance.y == "HR" ~ "g3",
...
umap_groups_recipe <- recipe(hr.group ~., data = data_groups) %>%
  ...
  step_orderNorm(all_numeric_predictors()) %>% 
  step_normalize(all_predictors()) %>% 
  step_umap(all_numeric_predictors(), outcome = "hr.group", num_comp = 4, min_dist = 0.01, neighbors = 25)
