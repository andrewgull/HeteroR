...
umap_rec <- orq_recipe %>%
  step_umap(all_numeric_predictors(), num_comp = 8, min_dist = 0.05, neighbors = 15) # the bigger the distance the more low level details you see in the clusters
...
If you want to use UMAP for prediction you shouldn't use the supervised version of UMAP

```{r, eval=F}
# change the recipe: add more components
# as many as the number of non-correlated predictors in NCORR recipe

umap_rec <- orq_recipe %>%
  step_umap(all_numeric_predictors(), num_comp = 20, min_dist = 0.05, neighbors = 15) # it breaks

data_umap <- prep(umap_rec, retain = TRUE)


write.csv(data_umap$template, "data/umap_hr123.csv", row.names = FALSE)
```
...
umap_rec_sup <- orq_recipe %>%
  step_umap(all_numeric_predictors(), outcome = "resistance", num_comp = 10, min_dist = 0.1, neighbors = 25)
...
data_umap_sup$template %>% 
  plot_validation_results() +
  ggtitle("UMAP supervised (HR I+II+III)")
```
...
umap_groups_recipe <- recipe(hr.group ~., data = data_groups) %>%
  ...
  step_orderNorm(all_numeric_predictors()) %>% 
  step_normalize(all_predictors()) %>% 
  step_umap(all_numeric_predictors(), outcome = "hr.group", num_comp = 6, min_dist = 0.02, neighbors = 25)
