...
```{r, warning=FALSE, fig.width=9, fig.height=9, eval=F}
...
```{r}
Fig_6A <- ggplot(data_strain, aes(n.beta.lac)) +
...
```{r}
Fig_6B <- ggplot(data_strain, aes(n.beta.lac.4)) +
  geom_bar(aes(fill = resistance), position = "fill",width = 0.3) +
  scale_fill_brewer(palette="Set1") +
  xlab("n β-lactamases") +
...
```{r}
Fig_6 <- ggarrange(Fig_6A , Fig_6B + ylab("") ,
                    labels = c("A", "B"),
...
Fig_7B <- ggplot(bl_type_count_tidy, aes(gene, sum))+
...
```{r}
Fig_7B <- ggplot(bl_type_count, aes(non.ampC)) +
  geom_bar(aes(fill = resistance), position = "dodge", alpha = 1) +
  scale_fill_brewer(palette = "Set1", name = "") +
...
```{r}
Fig_7C <- ggplot(data_strain, aes(TEM.beta.lactamase)) +
  geom_bar(aes(fill = resistance), position = "fill", alpha = 1) +
  scale_fill_brewer(palette = "Set1", name = "") +
...
legend_plot <- get_legend(Fig_7B)    
# Arrange the plots and the extracted legend
Fig_7 <- ggarrange(Fig_7A+ theme(legend.position = "none"), Fig_7B+ theme(legend.position = "none"), Fig_7C+ theme(legend.position = "none"), legend_plot,
                   labels = c("A", "B", "C"),
                   ncol = 2, nrow = 2)
...
```{r}
summary(glm(n.plasmids ~ coverage, data=data_strain, family = "poisson"))
...
```{r}
max_rep_len %>% filter(location == "max.rep.len.plasmid",
                       max.len > 0) %>%
  ggplot(aes(resistance, max.len)) +
  geom_violin(aes(fill = resistance), alpha = 0.7) +
  geom_boxplot(
...
Fig_5A <-
  rep_count %>% 
  #filter(n.rep > 0) %>% 
  ggplot(aes(resistance, log10(n.rep + 1))) +  # Add 1 to n.rep before log10
  geom_violin(aes(fill = resistance), alpha = 0.7) +
  geom_boxplot(
    aes(fill = resistance),
    alpha = 0.2,
  ...
  scale_fill_brewer(palette = "Pastel1") +
  facet_grid(cols = vars(location), labeller = as_labeller(facet_names)) +
  ylab("log10(n repeats + 1)") +  # Update label
  xlab("") +
  guides(fill = "none")
Fig_5A
```
