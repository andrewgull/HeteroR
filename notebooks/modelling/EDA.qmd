...
hr_testing12 <- read_csv("data/heteroresistance_testing_gr12.csv")
hr_testing13 <- read_csv("data/heteroresistance_testing_gr13.csv")
...
hr_testing <- left_join(hr_testing12, hr_testing13, by = "strain") %>% 
  mutate(resistance = case_when(resistance.x == "HR" | resistance.y == "HR" ~ "HR",
                              ...
                              is.na(resistance.x) & resistance.y == "HR" ~ "HR",
                              resistance.x == "HR" & is.na(resistance.y) ~ "HR",
...
For the scheme 1+3, there are 52 HR, 406 nonHR, 13 R and 50 NAs

For the scheme 1+2+3 there are 88 HR and 370 nonHR, 13 R and 50 NAs

I will start with the scheme 1+2, because it is more clinically relevant.

But the next scheme to test will be 1+2+3, because it has more HR (obviously)
...
data_pca$template %>% 
  ...
  ggtitle("Principal Component Analysis (HR123)")
```
...
Here I increase the minimal distance to see the local structure better

```{r, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
## UMAP Supervised

umap_rec_sup <- orq_recipe %>%
  step_umap(all_numeric_predictors(), outcome = "resistance", num_comp = 8, min_dist = 0.6, neighbors = 25)
...
umap3d <-
  plot_ly(
    ...
    colors = c('#cf280c', '#1b56f7')
    #symbol = data_umap_sup$template$prediction,
...
```{r}
write.csv(data_umap_sup$template, "data/umap_hr123.csv", row.names = FALSE)
```

## UMAP on separate HR groups

```{r}
hr_testing_12 <- read_csv("data/heteroresistance_testing_gr12.csv")
hr_testing_13 <- read_csv("data/heteroresistance_testing_gr13.csv")

hr_groups <- left_join(hr_testing_12, hr_testing_13, by = "strain") %>% 
...
umap_groups_recipe <- recipe(hr.group ~., data = data_groups) %>%
  ...
  step_orderNorm(all_numeric_predictors()) %>% 
  step_normalize(all_predictors()) %>% 
  step_umap(all_numeric_predictors(), outcome = "hr.group", num_comp = 8, min_dist = 0.6, neighbors = 25)
...
data_umap_groups$template %>% 
    ...
    geom_point(alpha = 0.4, size = 2, aes(shape = hr.group)) +
    #geom_autodensity(alpha = .3) +
    facet_matrix(vars(-hr.group), layer.diag = 2) + 
    scale_color_brewer(palette = "Set1", direction = 1) + 
    #scale_fill_brewer(palette = "Set1", direction = 1) +
    theme_bw() +
    ggtitle("Supervised UMAP (HR groups)")
```
