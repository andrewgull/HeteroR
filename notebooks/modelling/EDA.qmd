...
umap_rec_sup <- data_rec %>%
  step_umap(all_numeric_predictors(), outcome = "resistance", num_comp = 3)
...
fig2 <- plot_ly(umap_df, x = ~UMAP1, y = ~UMAP2, z = ~UMAP3, color = ~umap_df$resistance, colors = c('#cf280c', '#1b56f7')) 
fig2 <- fig2 %>% add_markers(size = 2, text = ~umap_df$strain) 
fig2 <- fig2 %>% layout(scene = list(xaxis = list(title = 'UMAP1'), 
