...
```{r, message=FALSE}
data <- read_csv("data/features_bl.csv.gz")
...
# without resistance
cor_matrix <- cor(data_strain %>% 
                    select(-c(strain, resistance, n.beta.lac.3, n.beta.lac.4)),
...
`n.plasmids` does not behasve well after transformation.
...
plot3d(x = umap_df$UMAP03, y = umap_df$UMAP06, z = umap_df$UMAP05,
       col = umap_df$color,
       type = "s", 
       radius = 0.05,
       xlab = "UMAP3",
