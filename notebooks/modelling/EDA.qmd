...
# TMP SOLUTION to add plasmid copy number
...
data_strain <- data_strain %>% left_join(pl_copy_number, by="strain") %>% 
  ...
  relocate(plasmid.copy.number, .after = n.plasmids)
...
plots <- comp_plots("plasmid.copy.number", data_strain, data_prep)
```
...
```{r}
data_strain %>%
  ggplot(aes(coverage, plasmid.copy.number)) +
  geom_point(aes(color = resistance), alpha = 0.5) +
  scale_color_brewer(palette = "Set1") +
  ggtitle("") +
...
```{r}
data_strain %>%
  ggplot(aes(n.plasmids, plasmid.copy.number)) +
  geom_point(aes(color = resistance), alpha = 0.5) +
  scale_color_brewer(palette = "Set1") +
  ggtitle("") +
  ylab("cn") +
...
```{r}
data_strain %>%
  ggplot(aes(chrom.status, plasmid.copy.number)) +
