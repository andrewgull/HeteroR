---
title: "models_table"
output: html_document
date: "2023-06-27"
---

```{r}
library(readr)
library(tidymodels)
library(themis)
library(probably)
library(vip)
library(skimr)
library(stacks)
library(bestNormalize) # for ord QQ norm
library(tidyposterior) # for Bayesian ANOVA
#library(embed) # for UMAP
library(finetune) # for win-loss tuning
source("functions.R")
library(colino)

# path for models
models_path <- "/mnt/data/andrei/Data/HeteroR/results/models/scheme12/"

rfe_model <- rand_forest(mode = "classification") %>%
  set_engine("ranger", num.threads = 8, importance = "impurity")

#function to get
get_j_index <- function(res){res %>%   show_best("j_index",n =20) %>% filter(.config == (
    res %>% show_best("roc_auc", n = 1) %>% select(.config) %>%  as.character()
)) %>%  select(mean) %>% as.character()}
```
FROM modeling


# Read and process data

The data sets are the same as in EDA

```{r}
data_strain <- read_csv("data/features_strain.csv", na = c("NA", "-Inf")) %>% 
  filter(strain != "DA63310")

# TWO SCHEMES
hr_testing12 <- read_csv("data/heteroresistance_testing.csv", col_select = c(strain, Gr12)) %>% 
  filter(!is.na(strain)) %>% 
  dplyr::rename("resistance" = Gr12 )

hr_testing13 <- read_csv("data/heteroresistance_testing.csv", col_select = c(strain, Gr13)) %>% 
  filter(!is.na(strain)) %>% 
  dplyr::rename("resistance" = Gr13 )

# created during EDA
hr_testing <- read_csv("data/heteroresistance_testing_gr123.csv") 

# USE HR 1+2
data_strain <- data_strain %>% 
  left_join(hr_testing12, by = "strain")

data_strain %>% 
  group_by(resistance) %>% 
  count()
```

Add n.beta.lac \>4

```{r}
strains <- data_strain$strain

data_strain <- data_strain %>% 
  #mutate(n.beta.lac.3 = factor(ifelse(n.beta.lac > 3, "yes", "no"))) %>% 
  mutate(n.beta.lac.4 = factor(ifelse(n.beta.lac > 4, "yes", "no"))) %>% 
  relocate(n.beta.lac.4, .before = "n.plasmids") %>% 
  filter(resistance != "R") %>% 
  mutate(resistance = factor(resistance, levels = c("HR", "nonHR")),
         chrom.status = factor(chrom.status))

data_strain$N50 <- NULL
#data_strain$NA. <- NULL
data_strain[is.na(data_strain)] <- 0
```

# Data split

Stratified split

```{r}
set.seed(124)

data_split <- initial_split(data_strain, prop = 0.8, strata = resistance)

df_train <- training(data_split)
df_test <- testing(data_split)

cv_folds <- vfold_cv(df_train, strata = "resistance", v = 10, repeats = 10) 
```

# RECIPIES

```{r}
main_recipe <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)

ncorr_recipe <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = tune("corr_tune")) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)


pcayj_recipe <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_pca(all_predictors(), num_comp = tune()) %>%
  step_normalize(all_numeric_predictors())

yj_recipe <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = tune("corr_tune"))

ncorq_recipe <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = tune("corr_tune")) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)
```

# MODELS

```{r}
lr_en <- logistic_reg(
      penalty = tune(), 
      mixture = tune()) %>%
      set_engine("glmnet")
```


#LR 

## MAIN 

```{r}
lr_main <- readRDS(paste0(models_path, "lr_main_space_p30.rds"))

best_perf_hyp <- lr_main %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr_main)

my_mod <-  logistic_reg(
        penalty =  best_perf_hyp$penalty[1],
        mixture = 1) %>%
      set_engine("glmnet")

 my_wf <- workflow() %>%
  add_model(my_mod) %>%
  add_recipe(main_recipe)
 
best_params <- lr_main %>% 
    select_best("roc_auc")
 

final_model <- finalize_workflow(my_wf, best_params) %>%
  last_fit(data_split)

trained_mod <- final_model %>% 
  extract_workflow() %>% 
  pull_workflow_fit()

coefs <- coef(trained_mod$fit, s =  best_perf_hyp$penalty[1])



predictors_to_remove <- c("(Intercept)","resistance", "strain")

coefficients <- coefs%>% row.names()
n_predictors <- coefficients[coefs@i] %>% discard(~ .x %in% predictors_to_remove) %>% length 

final_tibble <- tibble(Model = "LR", Preprocessin= "MAIN", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = NA, n_predictors =  n_predictors)

final_tibble
```

## NCORR 
```{r}
lr_ncorr_res <- readRDS(paste0(models_path, "lr_ncorr_p20.rds"))

best_perf_hyp <- lr_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr_ncorr_res)

corr_tune <-best_perf_hyp$corr_tune[1]

#Rebuild the workflow to extract the number of selected features 
my_mod <-  logistic_reg(
        penalty =  best_perf_hyp$penalty[1],
        mixture = 1) %>%
      set_engine("glmnet")

my_rec <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)

 my_wf <- workflow() %>%
  add_model(my_mod) %>%
  add_recipe(my_rec)
 
best_params <- lr_ncorr_res %>% 
    select_best("roc_auc")
 

final_model <- finalize_workflow(my_wf, best_params) %>%
  last_fit(data_split)

trained_mod <- final_model %>% 
  extract_workflow() %>% 
  pull_workflow_fit()

coefs <- coef(trained_mod$fit, s = best_perf_hyp$penalty[1])

coefficients <- coefs %>% row.names()
n_predictors <- coefficients[coefs@i] %>% discard(~ .x %in% predictors_to_remove) %>% length 

tmp_tibble <- tibble(Model = "LR", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORQ

```{r}
lr_ncorq_res <- readRDS(paste0(models_path, "lr_ncorq_p30.rds"))

best_perf_hyp <- lr_ncorq_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr_ncorq_res)

corr_tune <-best_perf_hyp$corr_tune[1]

#Rebuild the workflow to extract the number of selected features 

my_mod <-  logistic_reg(
        penalty =  best_perf_hyp$penalty[1],
        mixture = 1) %>%
      set_engine("glmnet")

my_rec <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)

 my_wf <- workflow() %>%
  add_model(my_mod) %>%
  add_recipe(my_rec)
 
best_params <- lr_ncorq_res %>% 
    select_best("roc_auc")
 

final_model <- finalize_workflow(my_wf, best_params) %>%
  last_fit(data_split)

trained_mod <- final_model %>% 
  extract_workflow() %>% 
  pull_workflow_fit()

coefs <- coef(trained_mod$fit, s = best_perf_hyp$penalty[1])

coefficients <- coefs%>% row.names()
n_predictors <- coefficients[coefs@i] %>% discard(~ .x %in% predictors_to_remove) %>% length 

tmp_tibble <- tibble(Model = "LR", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## PCA + YJ recipe

```{r}
lr_pcayj_res <- readRDS(paste0(models_path, "lr_pcayj_p20.rds"))

best_perf_hyp <- lr_pcayj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr_pcayj_res)

corr_tune <- NA

#Rebuild the workflow to extract the number of selected features 

my_mod <-  logistic_reg(
        penalty =  best_perf_hyp$penalty[1],
        mixture = 1) %>%
      set_engine("glmnet")

my_rec <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)

my_wf <- workflow() %>%
  add_model(my_mod) %>%
  add_recipe(my_rec)
 
best_params <- lr_pcayj_res %>% 
    select_best("roc_auc")
 
final_model <- finalize_workflow(my_wf, best_params) %>%
  last_fit(data_split)

trained_mod <- final_model %>% 
  extract_workflow() %>% 
  pull_workflow_fit()

coefs <- coef(trained_mod$fit, s = best_perf_hyp$penalty[1])

coefficients <- coefs%>% row.names()
n_predictors <- coefficients[coefs@i] %>% discard(~ .x %in% predictors_to_remove) %>% length

tmp_tibble <- tibble(Model = "LR", Preprocessin= "PCA_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r}
lr_ncoryj_res <- readRDS(paste0(models_path, "lr_ncoryj_p20_test.rds"))

best_perf_hyp <- lr_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

#Rebuild the workflow to extract the number of selected features 
my_mod <-  logistic_reg(
        penalty =  best_perf_hyp$penalty[1],
        mixture = 1) %>%
      set_engine("glmnet")

my_rec <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune)

 my_wf <- workflow() %>%
  add_model(my_mod) %>%
  add_recipe(my_rec)
 
 
best_params <- lr_ncoryj_res %>% 
    select_best("roc_auc")
 

final_model <- finalize_workflow(my_wf, best_params) %>%
  last_fit(data_split)

trained_mod <- final_model %>% 
  extract_workflow() %>% 
  pull_workflow_fit()

coefs <- coef(trained_mod$fit, s = best_perf_hyp$penalty[1])

coefficients <- coefs%>% row.names()
n_predictors <- coefficients[coefs@i] %>% discard(~ .x %in% predictors_to_remove) %>% length

tmp_tibble <- tibble(Model = "LR", Preprocessin= " NCORR_YJ ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## Ridge NCORR + YJ 

```{r}
lr0_ncoryj_res <- readRDS(paste0(models_path, "lr0_ncoryj_p20.rds"))

best_perf_hyp <- lr0_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr0_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

# given that ridge pushes thowards zero but not totally to zero I don't need to redo all the model but just preprocessing

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% length()

tmp_tibble <- tibble(Model = "Ridge_LR", Preprocessin= "NCORR_YJ ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble

```

Elastic net NCORR + YJ recipe

```{r}
lr_en_ncoryj_res <- readRDS(paste0(models_path, "lr-en_ncoryj_p20.rds"))

best_perf_hyp <- lr_en_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lr_en_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

#Rebuild the workflow to extract the number of selected features 

my_mod  <- logistic_reg(
      penalty = best_perf_hyp$penalty[1],
      mixture = best_perf_hyp$mixture[1]) %>%
      set_engine("glmnet")

#check this 
my_rec <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune)

my_wf <- workflow() %>%
  add_model(my_mod) %>%
  add_recipe(my_rec)
 
best_params <- lr_en_ncoryj_res %>% 
    select_best("roc_auc")
 

final_model <- finalize_workflow(my_wf, best_params) %>%
  last_fit(data_split)

trained_mod <- final_model %>% 
  extract_workflow() %>% 
  pull_workflow_fit()

coefs <- coef(trained_mod$fit, s = best_perf_hyp$penalty[1])

coefficients <- coefs%>% row.names()
n_predictors <- coefficients[coefs@i] %>% discard(~ .x %in% predictors_to_remove) %>% length

tmp_tibble <- tibble(Model = "EL_LR", Preprocessin= " NCORR_YJ ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

# MARS

## NCORR 

```{r mars norm}
mars_ncorr_res <- readRDS(paste0(models_path, "mars_ncorr_p20.rds"))

best_perf_hyp <- mars_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mars_ncorr_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames()  %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MARS", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r mars norm}
mars_ncoryj_res <- readRDS(paste0(models_path, "mars_ncoryj_p20.rds"))

best_perf_hyp <- mars_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mars_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MARS", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORQ

```{r}
mars_ncorq_res <- readRDS(paste0(models_path, "mars_ncorq_p30.rds"))

best_perf_hyp <- mars_ncorq_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mars_ncorq_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MARS", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe

```{r}
mars_yjrfe_res <- readRDS(paste0(models_path, "mars_yjrfe_res_p30.rds"))

best_perf_hyp <- mars_yjrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mars_yjrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MARS", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## Bagged MARS + RFE + YJ recipe

```{r}
bag_mars_yjrfe_res <- readRDS(paste0(models_path, "bag_mars_yjrfe_res_p30.rds"))

best_perf_hyp <- bag_mars_yjrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(bag_mars_yjrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MARS_Bagged", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

# Linear support vector machines (lSVM)

## NCORR 

```{r}
lsvm_ncorr_res <- readRDS(paste0(models_path, "lsvm_ncorr_race_p30.rds"))

best_perf_hyp <- lsvm_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lsvm_ncorr_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "lSVM", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r}
lsvm_ncoryj_res <- readRDS(paste0(models_path, "lsvm_ncoryj_race_p30_test.rds"))

best_perf_hyp <- lsvm_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lsvm_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "lSVM", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORQ

```{r}
lsvm_ncorq_res <- readRDS(paste0(models_path, "lsvm_ncorq_race_p30.rds"))

best_perf_hyp <- lsvm_ncorq_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lsvm_ncorq_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "lSVM", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble, tmp_tibble)

final_tibble
```

## RFE + YJ recipe

```{r}
lsvm_yjrfe_res <- readRDS(paste0(models_path, "lsvm_yjrfe_res_p30.rds"))

best_perf_hyp <- lsvm_yjrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(lsvm_yjrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "lSVM", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe Bayesian grid tuning 

skipped

#Polynomial support vector machines (pSVM)

## NCORR 

```{r mars norm}
psvm_ncorr_res <- readRDS(paste0(models_path, "psvm_ncorr_race_p30.rds"))

best_perf_hyp <- psvm_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(psvm_ncorr_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "pSVM", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

checking how these 4 predictors of pSVM ncorrq perform on test

```{r}
blank_mod <-  svm_poly(
      cost = tune(),
      degree = tune(),
      scale_factor = tune(),
      margin = NULL) %>% # regression only
      set_mode("classification") %>%
      set_engine("kernlab", num.threads = cores)
best_svm <-  psvm_ncorr_res %>%  select_best() 
tuned_mod <- 
  blank_mod <-  svm_poly(
      cost =best_svm$cost,
      degree = best_svm$degree,
      scale_factor =best_svm$scale_factor,
      margin = NULL) %>% # regression only
      set_mode("classification") %>%
      set_engine("kernlab", num.threads = cores)

tuned_rec <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = best_svm$corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)

run_last_fit <-
  function(model_name,
           blank_spec,
           tuned_spec,
           recipe,
           train_test_split = data_split,
           return_fit = FALSE) {
    # last workflow
    last_wf <- workflow() %>%
      add_model(blank_spec) %>%
      add_recipe(recipe) %>%
      update_model(tuned_spec)
    
    set.seed(345)
    # final fit
    final_fit <- final_metrics <-
      last_wf %>%
      last_fit(train_test_split)
    # final metrics
    final_metrics <-
      final_fit %>%
      collect_metrics() %>%
      select(.metric, .estimate) %>%
      mutate(model = model_name)
    if (return_fit) {
      return(list(final_metrics, final_fit))
    } else {
      return(list(final_metrics, NULL))
    }
  }

run_last_fit("psvm_ncorr", blank_mod,tuned_mod,tuned_rec )
```

## NCORQ

```{r}
psvm_ncorq_res <- readRDS(paste0(models_path, "psvm_ncorq_race_p30.rds"))

best_perf_hyp <- psvm_ncorq_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(psvm_ncorq_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "pSVM", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r mars norm}
psvm_ncoryj_res <- readRDS(paste0(models_path, "psvm_ncoryj_race_p30_test.rds"))

best_perf_hyp <- psvm_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(psvm_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "pSVM", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## PCA + YJ 

```{r}
psvm_pca_res <- readRDS(paste0(models_path, "psvm_pcayj_race_p30.rds"))

best_perf_hyp <- psvm_pca_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(psvm_pca_res)

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_pca(all_predictors(), num_comp = best_perf_hyp$num_comp[1]) %>%
  step_normalize(all_numeric_predictors()) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()


tmp_tibble <- tibble(Model = "pSVM", Preprocessin= "PCA_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe

```{r}
psvm_yjrfe_res <- readRDS(paste0(models_path, "psvm_yjrfe_res_p30.rds"))

best_perf_hyp <- psvm_yjrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(psvm_yjrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "lSVM", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe Bayesian grid

skipped - very symilar to space

# Radial Basis Function SVM

## MAIN

```{r}
rbf_main_res <- readRDS(paste0(models_path, "rbfsvm_main_race_p30.rds"))

best_perf_hyp <- rbf_main_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rbf_main_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "rbfSVM", Preprocessin= "MAIN", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR 

```{r mars norm}
rbf_ncorr_res <- readRDS(paste0(models_path, "rbfsvm_ncorr_race_p30.rds"))

best_perf_hyp <- rbf_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rbf_ncorr_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "rbfSVM", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORQ

```{r}
rbf_ncorq_res <- readRDS(paste0(models_path, "rbfsvm_ncorq_race_p30.rds"))

best_perf_hyp <- rbf_ncorq_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rbf_ncorq_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "rbfSVM", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r}
rbf_ncoryj_res <- readRDS(paste0(models_path, "rbfsvm_ncoryj_race_p30.rds"))

best_perf_hyp <- rbf_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rbf_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "rbfSVM", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe

```{r}
rbfsvm_yjrfe_res <- readRDS(paste0(models_path, "rbfsvm_yjrfe_res_p30.rds"))

best_perf_hyp <- rbfsvm_yjrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rbfsvm_yjrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "rbfSVM", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe Bayesian grid tuning 

skipped

# k-Nearest Neighbors (KNN)

## NCORR 

```{r}
knn_ncorr_res <- readRDS(paste0(models_path, "knn_ncorr_race_p30.rds"))

best_perf_hyp <- knn_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(knn_ncorr_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "KNN", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORQ

```{r}
knn_ncorq_res <- readRDS(paste0(models_path, "knn_ncorq_race_p30.rds"))

best_perf_hyp <- knn_ncorq_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(knn_ncorq_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "KNN", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r}
knn_ncoryj_res <- readRDS(paste0(models_path, "knn_ncoryj_race_p30.rds"))

best_perf_hyp <- knn_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(knn_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "KNN", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + YJ recipe

```{r}
knn_yjrfe_res <- readRDS(paste0(models_path, "knn_yjrfe_res_p30.rds"))

best_perf_hyp <- knn_yjrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(knn_yjrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "KNN", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

# Random Forest (RF)

## MAIN

```{r}
rf_main_res <- readRDS(paste0(models_path, "rf_main_race_p30.rds"))

best_perf_hyp <- rf_main_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rf_main_res)

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "RF", Preprocessin= "MAIN", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR 

```{r mars norm}
rf_ncorr_res <- readRDS(paste0(models_path, "rf_ncorr_p20.rds"))

best_perf_hyp <- rf_ncorr_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rf_ncorr_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "RF", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r}
rf_ncoryj_res <- readRDS(paste0(models_path, "rf_ncoryj_race_p30.rds"))

best_perf_hyp <- rf_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rf_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "RF", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + main recipe

```{r}
rf_mainrfe_res <- readRDS(paste0(models_path, "rf_mainrfe_res_p30.rds"))

best_perf_hyp <- rf_mainrfe_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rf_mainrfe_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "RF", Preprocessin= "MAIN_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## MAIN Boruta 

```{r}
rf_mainbor_res <- readRDS(paste0(models_path, "rf_mainbor_p30.rds"))

best_perf_hyp <- rf_mainbor_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rf_mainbor_res)

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_select_boruta(all_predictors(), outcome = "resistance") %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "RF", Preprocessin= "MAIN_Boruta", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```
Is it possible that 
boruta in RF selects all of the params?

```{r}
library(Boruta)

bor_df <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)  %>% prep() %>% juice() %>% select(-strain)

b <- Boruta(resistance ~ ., data = bor_df)

plot(b)
```

# Boosted Trees (BT)

## MAIN

```{r}
rf_main_res <- readRDS(paste0(models_path, "rf_main_race_p30.rds"))

best_perf_hyp <- rf_main_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(rf_main_res)

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "BT", Preprocessin= "MAIN", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + win-loss 

```{r}
xgb_main_wl <- readRDS(paste0(models_path, "xgb_main_race_p30.rds"))

best_perf_hyp <- xgb_main_wl %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(xgb_main_wl)

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "BT", Preprocessin= "NCORR_WL", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + Bayes

```{r}
xgb_ncorr_bres <- readRDS(paste0(models_path, "xgb_ncorr_bres_p30_test.rds"))

best_perf_hyp <- xgb_ncorr_bres %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <- NA

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  #step_corr(threshold = opt$corr_threshold) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "BT", Preprocessin= "NCORR", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## RFE + main recipe + Bayesian search

```{r}
xgb_mainrfe_bayes <- readRDS(paste0(models_path, "bt_mainrfe_bres_p30.rds"))

best_perf_hyp <- xgb_mainrfe_bayes %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <- NA

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100)%>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "BT", Preprocessin= "MAIN_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## MAIN Boruta 

```{r}
xgb_main_bor_bres <- readRDS(paste0(models_path, "bt_mainbor_bres_p30.rds"))

best_perf_hyp <- xgb_main_bor_bres %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <- NA

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_select_boruta(all_predictors(), outcome = "resistance") %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "BT", Preprocessin= "MAIN_Boruta", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## MAIN + YJ-transform Boruta 

```{r}
xgb_yjbor_bres <- readRDS(paste0(models_path, "xgb_yjbor_bres_p30.rds"))

best_perf_hyp <- xgb_yjbor_bres %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <- NA

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_boruta(all_predictors(), outcome = "resistance") %>% prep() %>% juice() %>% colnames()%>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "BT", Preprocessin= "MAIN_YJ_Boruta", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

# Multilayer perceptron (MLP NNET)

## NCORQ 

```{r}
# win-loss here
mlp_ncorq_wl <- readRDS(paste0(models_path, "mlp_nnet_ncorq_race_p30.rds"))

best_perf_hyp <- mlp_ncorq_wl %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mlp_ncorq_wl)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <-  recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MLP", Preprocessin= "NCORQ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## PCA + YJ 

```{r}
mlp_pca <- readRDS(paste0(models_path, "mlp_nnet_pcayj_p20.rds"))

best_perf_hyp <- mlp_pca %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mlp_pca)

corr_tune <- NA

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_pca(all_predictors(), num_comp = best_perf_hyp$num_comp[1]) %>%
  step_normalize(all_numeric_predictors()) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()


tmp_tibble <- tibble(Model = "MLP", Preprocessin= "PCA_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## NCORR + YJ 

```{r}
mlp_ncoryj_res <- readRDS(paste0(models_path, "mlp_ncoryj_p20_test.rds"))

best_perf_hyp <- mlp_ncoryj_res %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mlp_ncoryj_res)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MLP", Preprocessin= "NCORR_YJ", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## Keras + PCA + YJ

skipped - the same

## RFE + YJ recipe

```{r}
mlp_yjrfe_wl <- readRDS(paste0(models_path, "mlp_yjrfe_res_p30.rds"))

best_perf_hyp <- mlp_yjrfe_wl %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mlp_yjrfe_wl)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "MLP", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

final_tibble
```

## Bag MLP + RFE + YJ recipe

```{r}
mlp_bag_yjrfe_wl <- readRDS(paste0(models_path, "bag_mlp_yjrfe_res_p30.rds"))


best_perf_hyp <- mlp_bag_yjrfe_wl %>% show_best(n=20 )

mean_auc <- best_perf_hyp$mean[1]

st_error <- best_perf_hyp$std_err[1]

mean_j_index <-get_j_index(mlp_bag_yjrfe_wl)

corr_tune <- best_perf_hyp$corr_tune[1]

n_predictors <- recipe(resistance ~ ., data = df_train) %>%
  update_role(strain, new_role = "ID") %>%
  step_nzv(all_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_YeoJohnson(all_numeric_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_smote(resistance, over_ratio = 1, seed = 100) %>%
  step_corr(all_predictors(), threshold = corr_tune) %>%
  step_select_vip(
    all_numeric_predictors(),
    outcome = "resistance",
    model = rfe_model,
    threshold = .9
  ) %>% prep() %>% juice() %>% colnames() %>% discard(~ .x %in% predictors_to_remove) %>% length()

tmp_tibble <- tibble(Model = "Bag_MLP", Preprocessin= "NCORR_YJ_RFE", mean_AUC = mean_auc, st_error = st_error, mean_j_index = mean_j_index, Correlation = corr_tune, n_predictors =  n_predictors)

final_tibble <- bind_rows(final_tibble,tmp_tibble)

# add rounding
final_tibble <-
  final_tibble %>%
  mutate(
    mean_AUC = round(mean_AUC, 3),
    mean_j_index = round(mean_j_index, 3),
    Correlation = round(Correlation, 2),
    st_error = round(st_error, 3)
  )

final_tibble
```






