...
With the 1st classification and a single best model (XGBTree), I got
decent results:
...
data_strain <- read_csv("data/features_bl_strain.csv", na = c("NA", "-Inf")) %>% select(-resistance) # remove select when features.Rmd is fixed!

# TWO SCHEMES IN TWO FILES
hr_testing <- read_csv("data/heteroresistance_testing_gr13.csv")
...
The data set is not yet complete.
...
# without resistance
cor_matrix <- cor(data_strain %>% select(-c(strain, resistance, n.beta.lac.3, n.beta.lac.4)), use = "pairwise.complete.obs")
...
> while high values will push UMAP towards representing the big-picture
