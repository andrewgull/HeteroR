...
count_genes <- function(x) {
  ...
  df2 <-
    df_UC_FMP_all %>% filter(strain == x) %>%  select(strain, plasmids, size) %>% dplyr::rename(n_gene_copies = plasmids, length_d = size) %>% arrange(desc(length_d))
  ...
  total_df <- cbind(df1, df2) %>%
    ...
    ) %>% select(chr, strain, n_gene_copies)
...
gene_counts_rgi <- map_dfr(strains, ~ count_genes(.))
```
...
```{r}
features <-
  gene_counts_rgi %>% select(record_id, strain, n_gene_copies) %>% right_join(features, by = c("strain", "record_id"))
```
...
repeat_counts <- function(features_selected, ar) {
  ...
  ar_colnames <-
    c(
      ...
      paste0("nc.rep.plasmid.", ar),
      paste0("nc.rep.total.", ar)
    )
  ...
  features_ar <- features_selected %>%
    ...
    dplyr::rename(strain = strain,
                  n.rep.chrom = no,
                  n.rep.plasmid = yes) %>%
    select(strain, n.rep.chrom, n.rep.plasmid) %>% merge(features_strain %>% select(strain, plasmid.copy.number))  %>%
    mutate(
      nc.rep.plasmid = n.rep.plasmid  * plasmid.copy.number,
      nc.rep.total = n.rep.chrom + n.rep.plasmid
    ) %>% select(-plasmid.copy.number) %>% setNames(nm = ar_colnames)
...
features_strain <- 
  list(features_strain, is_types, max_len, families_pivot, is_all) %>% 
  reduce(left_join, by = "strain") %>% 
  as_tibble() #??
```
