...
df_UC_short_no_scaffold_all <-
  map_dfr(UC_short_no_scaffold, ~ get_cov_uc_header(.))
...
```{r}
features <-
  gene_counts_rgi %>% select(record_id, strain, nc.plasmids) %>% right_join(features, by = c("strain", "record_id"))
```
...
n_genes_plasmids <-
  gff_with_counts %>% 
  mutate(n_genes_on_plasmid = ifelse(on.plasmid == "yes", nc.plasmids, 0)) %>% 
  group_by(strain) %>% 
  summarise(n.beta.lac.plasmid = sum(n_genes_on_plasmid))
...
med_ar_len_ampC <- med_ar_len_split(features_ampC,"ampC")
...
tem_selected_seqs <- map(TEM_strains, ~ tem_select(.))
...
med_ar_len_CTX <- med_ar_len_split(features_CTX, "CTX")
...
med_ar_len_TEM <- med_ar_len_split(features_TEM,"TEM")
...
```{r}
#a ll of the SHV resistance genes are on plasmids, can't use the function
...
```{r}
# universal functions
med_ar_len_split_SHV <- med_ar_len_split(features_SHV, "SHV")

med_ar_len_SHV <- med_ar_len_split(features_SHV, "SHV")
...
min_distance_is_gene <-
  features %>% select(strain, record_id, gene.length, gene_center) %>%  distinct %>%  left_join((
    isescan_df %>% dplyr::rename(record_id = seqID) %>% mutate(isCenter = (isBegin + isEnd) /
                                                                 2)
  ),
  by = c("record_id", "strain")) %>%  mutate(
    is_gene_distance = abs(isCenter - gene_center),
    is_inside_gene = ifelse(is_gene_distance < (gene.length / 2), TRUE, FALSE)
  )  %>%  filter(!is_inside_gene) %>% group_by(strain) %>% summarise(min_distance_is_gene = min(is_gene_distance)) 
...
interrupted_genes <-
  ...
  by = c("record_id", "strain")) %>%  mutate(
    is_gene_distance = abs(isCenter - gene_center),
    is_inside_gene = ifelse(is_gene_distance < (gene.length / 2), TRUE, FALSE)
  ) %>% group_by(strain) %>%  summarise(n.interrupted.genes = sum(as.numeric(is_inside_gene)))
