...
```{r, message=FALSE}
ass_sum_path <- "/mnt/data/andrei/Data/HeteroR/results/assemblies_joined/"

assemblies <- future_map_dfr(strains, function(x){
  read_delim(paste0(ass_sum_path, x, "/summary.tsv"), col_types = "cccccccccccc")}) %>%
...
gff_beta_lac <- features %>% 
  ...
  distinct() %>% 
  right_join(gff_beta_lac, by = "record_id")
```
...
gff_with_counts <- gene_counts_rgi %>% 
  select(record_id, strain, nc.plasmids) %>% 
  right_join(gff_beta_lac, by = c("record_id", "strain"))
