...
Alternative (if I want to have info on sub-types of beta-lactamases and SNPs):

```{r}
#| eval: true

features <- features %>% 
  select(-c(Best_Hit_ARO, SNPs_in_Best_Hit_ARO, Other_SNPs))
...
Alternative (if filtering by Resistance mechanism was not applied):

```{r}
#| eval: true

min_dist_oriC_bl <- gff %>% 
  filter(is.beta.lac) %>% 
  group_by(strain, on.plasmid) %>% 
  summarise(min.dist.oriC = min(dist.to.oriC)) %>% 
  spread(key = "on.plasmid", value = "min.dist.oriC", fill = 0) %>% # FILL SHOULD BE NA BUT YOU CAN NOT ADD NA TO NUMBER
  rename("min.dist.oriC.chrom.bl" = no, "min.dist.oriC.plasmid.bl" = yes) %>% 
  mutate(min.dist.oriC.bl = min.dist.oriC.chrom.bl + min.dist.oriC.plasmid.bl)

load("/home/andrei/Data/HeteroR/notebooks/features_soft.RData")
```
...
```{r, eval=TRUE}
save.image(f = "/home/andrei/Data/HeteroR/notebooks/features_soft.RData")
```
