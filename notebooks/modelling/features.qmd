...
df_UC_short_raw <- map_dfr(short_UC, ~ read_depth_short(.))
df_UC_short_raw
```
...
df_UC_short_scaffold_all <-
  map_dfr(UC_short_scaffold, ~ short_UC_scaffold_plasmids_count(.))
df_UC_short_scaffold_all
```
...
plasmid_copy_number <- rbind(df_UC,
                             ...
                             df_FMP,
                             df_FMP_short) %>% 
  rename(n.c.plasmids = tot_plasmids)
```
...
not all strains got the ISscan treatment,
...
isescan_df <- map_dfr(is_strains, ~ read_isescan_csv(.))
...
is_types <- isescan_df  %>%
  ...
  group_by(strain) %>%
  summarise(unique_count = n()) %>%  dplyr::rename(n_IS_families = unique_count)

#total amount of IS per strain
is_all <- isescan_df  %>%
  group_by(strain) %>%
  summarise(count = n()) %>%  dplyr::rename(tot_n_IS = count)

# per strain max length 
max_len <- isescan_df   %>% filter(family != "new" ) %>% 
  group_by(strain)%>%
  summarise(max_islen = max(isLen)) 
...
features_strain %>% left_join(is_types, by = "strain") %>% merge(max_len, by = "strain") %>% merge(families_pivot , by = "strain")
```

Are there any genes that have the same insertion sequence family on both sides?

```{r}
features %>% select(strain,record_id,gene.length,gene_center) %>%  distinct %>% left_join((isescan_df %>% dplyr::rename(record_id=seqID)),by= c("record_id","strain") ) %>%  mutate(isCenter = (isBegin + isEnd)/2,is_gene_distance = abs(isCenter-gene_center), is_inside_gene = ifelse(is_gene_distance < (gene.length/2),TRUE,FALSE)) %>% filter(is_inside_gene)%>%
