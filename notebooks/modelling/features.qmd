...
```{r, message=FALSE}
coverage <- read_delim("/mnt/data/andrei/Data/HeteroR/results/coverage/strains_475.tsv", na = c("", "NaN")) %>% 
...
```{r}
df_UC_all %>% group_by(strain) %>%  filter(size == max(size)) %>%  ungroup() %>%  pull(plasmids) %>%  as.numeric() %>%  table
```
...
```{r}
df_UC_short_no_scaffold_all %>% group_by(Strain) %>%  filter(Length == max(Length)) %>%  ungroup() %>%  pull(plasmids) %>%  table
```
...
```{r}
df_UC_short_scaffold_all %>% group_by(Strain) %>%  filter(Length == max(Length)) %>%  ungroup() %>%  pull(plasmids) %>%  table
```
...
build_short_FMP <- function(x) {
  ...
  total_df <- cbind(df1, df2) %>%
    mutate(
      ...
      -percent_diff,-next_max_length,-prev_max_length,-max_diff_next,-max_diff_prev,-is_lower_than_next,-is_lower_than_prev
    )
...
```{r}
df_FMP_short_all %>% group_by(Strain) %>%  filter(max_length == max(max_length)) %>%  ungroup() %>%  pull(plasmids) %>%  table
```
