...
```{r constants}
annot_dir <- "/mnt/data/andrei/Data/HeteroR/results/annotations/"
rep_bed_dir <- "/mnt/data/andrei/Data/HeteroR/results/direct_repeats/"
res_genes_dir <- "/mnt/data/andrei/Data/HeteroR/results/resistance_genes/"
coverage_table <- "/mnt/data/andrei/Data/HeteroR/results/coverage/strains_475.tsv"
ass_joined_path <- "/mnt/data/andrei/Data/HeteroR/results/assemblies_joined/"
coverage_dir <- "/mnt/data/andrei/Data/HeteroR/results/genome_coverage/"
isescan_dir <- "/mnt/data/andrei/Data/HeteroR/results/isescan/"
...
strains <-
  dir("/mnt/data/andrei/Data/HeteroR/results/direct_repeats")

# read lab testing data and filter out R strains
...
Strains that don't have repeats: `r length(setdiff(strains, unique(repeat_df$strain)))`
...
bed_df <- future_map_dfr(strains, function(x) {
  path <-
    paste0(
      rep_bed_dir,
      x,
      "/regions/regions_within.bed"
    )
  df <-
    read_delim(path, col_names = FALSE, col_types = "cddcdd") %>% mutate(strain = x)
  return(df)
}) %>%
...
features <- left_join(repeat_df, bed_df, by = "record_id") %>%
  mutate(
    spans_center = if_else(end_1 <= gene_center &
                             start_2 >= gene_center, "yes", "no"),
    record_id = sub("_gene", "", record_id)
  ) %>%
  select(-strain.y) %>%
  rename(strain.x = "strain",
...
rgi <- future_map_dfr(strains, function(x) {
  ...
  mutate(record_id = map_chr(ORF_ID, function(x)
    strsplit(x, " ")[[1]][1])) %>%
...
RGI - Features: `r length(setdiff(rgi$record_id, features$record_id))`

That's how many RGs don't have repeats around them.
...
### Filtering by Percentage Length:
...
Filtered RGI - Features: `r length(setdiff(rgi$record_id, features$record_id))` resistance genes after filtering don't have repeats around them.
...
That's how many strains there are in features: `r length(unique(features$strain))`
...
workflow/scripts/coverage_cli.py resources/strain_lists/strains_475.txt 5131220 results/coverage/strains_475.tsv filtered
```

Read the coverage table

```{r, message=FALSE}
coverage <- read_delim(coverage_table, na = c("", "NaN")) %>% 
...
`completeness` and `N50` come from assemblies summary files; keep only strains used in the analysis

N50 is available only for Unicycler-assembled genomes therefore not useful

```{r, message=FALSE}
assemblies <- future_map_dfr(strains, function(x){
  read_delim(paste0(ass_joined_path, x, "/summary.tsv"), col_types = "cccccccccccc")}) %>%
...
Join `assemblies` and `coverage` tables to make per strain features; keep only strains that were used in the analysis

## Create per-strain features table

```{r}
features_strain <- assemblies %>%
  filter(Type == "Chromosome") %>%
  select(Strain, Status2) %>%
  rename(Strain = "strain",
         Status2 = "chrom.status") %>%
  left_join(coverage, by = "strain") %>%
  rename(max_len = "read.max.len") 
...
checking if all chromosomes have count == 1
...
short_FMP <- short_FMP[!is.na(short_FMP)]
...
```{r}
df_FMP_all %>% group_by(strain) %>%  filter(size == max(size)) %>%  ungroup() %>%  pull(plasmids) %>%  table
```

#### Short Unicycler assemblies

```{r, eval=FALSE}
# short_UC

read_depth_short <- function(x) {
   path <- paste0(coverage_dir, x, "/depth.txt")
...
build_short_FMP <- function(x) {
  # x is a strain name like DA00000
...
short_FMP[short_FMP != "DA63044"]
df_FMP_short_all <- future_map_dfr(short_FMP[short_FMP != "DA63044"], ~ build_short_FMP(.))
```
...
df_FMP_short_all %>%
  group_by(Strain) %>%
  filter(max_length == max(max_length)) %>%
  ungroup() %>%
  pull(plasmids) %>%
  table()
```
...
plasmid_copy_number <- rbind(df_UC,
                             ...
                             df_FMP,
                             df_FMP_short) %>% 
  rename(plasmid.copy.number = "n.c.plasmids")
```
...
# If keep DA63044, then
# Error in data.frame(..., check.names = FALSE) :
# arguments imply differing number of rows: 96, 87
gene_counts_rgi <- future_map_dfr(strains[strains != "DA63044"], ~ count_genes(.))
```
...
**N.B.** gene orientation and distance to *oriC* are not possible to infer from our data!

Discard them in the following code.
...
gff_files <-
  map_chr(strains, ~ str_c(annot_dir, ., "/prokka/", ., "_genomic.gff"))
...
gff <- future_map_dfr(gff_files, ~ read_gff(.)) %>%
  ...
  # dist.to.oriC, pos.strand have been removed from select()
  select(record_id, on.plasmid, gene.length)
...
## Add gff table (columns: 'on.plasmid', 'strand' and 'dist.to.oriC') to `features`

```{r}
features <- features %>% 
  # add is.beta.lac again
  # but now both TRUE/FALSE
  mutate(is.beta.lac = grepl("beta-lactamase", AMR.Gene.Family)) %>% 
...
## Add 'strain' to `gff` (GFF with BL genes)
...
-   minimum distance to oriC - NOT RELEVANT

-   average (median) distance to oriC - NOT RELEVANT

-   number of genes on a positive strand - NOT RELEVANT
...
features_strain <-
  left_join(features_strain,
            n_genes_plasmids, 
            by = "strain") %>%
  mutate(n.beta.lac.chrom = n.beta.lac - n.beta.lac.plasmid) %>%
  relocate(n.beta.lac.plasmid, n.beta.lac.chrom)
...
rep_counts_center <- features %>% 
  ...
  rename(no = "n.rep.cen.chrom",
         yes = "n.rep.cen.plasmid") %>% 
...
repeat_counts_strain_100 <- features %>% 
  ...
  rename(no = "n.rep.100.chrom", 
         yes = "n.rep.100.plasmid") %>% 
...
repeat_counts_strain_100_center <- features %>% 
  ...
  rename(no = "n.rep.100.cen.chrom", yes = "n.rep.100.cen.plasmid") %>% 
...
# locate
features <- features %>% 
  relocate(n_repeats_gene_min100, .before="ORF_ID")
...
repeat_counts_strain_500 <- features %>% 
  ...
  rename(no = "n.rep.500.chrom",  
        yes = "n.rep.500.plasmid") %>% 
...
repeat_counts_strain_500_center <- features %>% 
  ...
  rename(no = "n.rep.500.cen.chrom", 
         yes = "n.rep.500.cen.plasmid") %>% 
...
repeats_centered_files <-
  map_chr(strains,
          ~ str_c(annot_dir, ., "/repeats/", ., "_repeats_centered.csv"))
...
repeats_centered <-
  future_map_dfr(repeats_centered_files,
                 ~ read_csv_file(., show_col_types = FALSE)) %>%
...
repeats_centered <-
  ...
  # filter(!is.na(start_1) | !is.na(start_2)) %>% 
  group_by(strain) %>%
  summarise("n" = sum(n())) %>%
  # dplyr::rename(strain = strain, rep.in = n) # what is this name? (AG)
  dplyr::rename(strain = strain, rep.tot.cen = n)
...
rep_len_sum_strain_split <- features %>% 
  ...
  rename(strain = "strain", 
         no = "med.tot.rep.len.chrom", 
         yes = "med.tot.rep.len.plasmid") %>% 
...
med_rep_len_split <- features %>% 
  ...
  rename(no = "med.rep.len.chrom", 
         yes = "med.rep.len.plasmid") %>% 
...
max_rep_len_all_split <- features %>% 
  ...
  rename(no = "max.rep.len.chrom", 
         yes = "max.rep.len.plasmid") %>% 
...
max_rep_len_cen_split <- features %>%
  ...
  summarize(max.rep.len = max(repeat_length, na.rm = T)) %>% 
  spread(key = "on.plasmid", value = "max.rep.len", fill = 0) %>% 
  rename(no = "max.rep.len.cen.chrom", 
         yes = "max.rep.len.cen.plasmid") %>% 
...
med_ar_len_split <- features %>% 
  ...
  rename(no = "med.AR.len.chrom", 
         yes = "med.AR.len.plasmid") %>% 
...
med_ar_len_cen_split <- features %>% 
  ...
  rename(no = "med.AR.len.cen.chrom", 
         yes = "med.AR.len.cen.plasmid") %>% 
...
min_ar_len_cen_split <- features %>% 
  ...
  rename(no = "min.AR.len.cen.chrom", 
         yes = "min.AR.len.cen.plasmid") %>% 
...
In real life, every E.coli has at least 1 ampC gene on chromosome - the table above should be empty, unless `AMR.Gene.Family` is `NA`!
...
amr_gene_presence <- relocate(amr_gene_presence, strain, .before = 1)
...
we have 6 differnet sequences, how many different families do we have?
...
We have 2 families, how many different AA sequences do we have?
...
ok, one sequence. no need to fix anything
...
tem_pivot <- TEM_rgi %>%
  ...
  # check the available TEM names here
  dplyr::rename(n.TEM.1 = `TEM-1`, 
                n.TEM.207 = `TEM-207`)
...
```{r}
is_strains <- dir(isescan_dir)
is_strains <- is_strains[is_strains %in% strains]
```
...
# Check if file exists
  if (!file.exists(path)) {
    ...
    # Create a tibble with specified column names and NA values
    df <- tibble(
      seqID = NA, family = NA, cluster = NA, isBegin = NA, isEnd = NA, isLen = NA,
      ncopy4is = NA, start1 = NA, end1 = NA, start2 = NA, end2 = NA, score = NA,
      irId = NA, irLen = NA, nGaps = NA, orfBegin = NA, orfEnd = NA, strand = NA,
      orfLen = NA, `E-value` = NA, `E-value4copy` = NA, type = NA, ov = NA, tir = NA
    )
  } else {
...
sus_strains <- max_is_len %>% 
  slice_head(n = 10) %>% 
  pull(strain)

norm_strains <- max_is_len %>% 
  slice_tail(n = 10) %>% 
  pull(strain)

plot_fun <- function(str_name) {
  filter(isescan_df, strain == str_name) %>%
    ggplot(aes(isLen, orfLen)) +
    geom_point() +
    ggtitle(str_name)
}

orf_len_plots <- map(c(sus_strains, norm_strains), ~ plot_fun(.))

ggpubr::ggarrange(plotlist = orf_len_plots, nrow = 5, ncol = 2)
```

As you can see, the longest IS elements sometimes have too short ORFs, moreover they always code for many ORFs not related to Tn and they are never compound IS elements (various genes flanked by Tn domains). For deeper analysis see `isescan_control.qmd`.

Such pseudo-IS (false positives) should be filtered out. Max length for IS and ORF = 2000 nt.

### per strain features

```{r}
max_len_allowed <- 2000

# filter the main IS table
isescan_df <- isescan_df %>%
  filter(family != "new",
         isLen <= max_len_allowed,
         orfLen <= max_len_allowed)

# families per strain
is_types <- isescan_df  %>%
  count(family, strain, name = "count") %>%
  group_by(strain) %>%
  summarise(n.is.fam = n())

# total amount of IS per strain
is_all <- isescan_df  %>%
  group_by(strain) %>%
  summarise(n.is.tot = n())
...
# if features do not have 'gene.length', add it from gff: 
# features <- features %>% left_join(gff %>% select(-on.plasmid), by = c("strain", "record_id"))
