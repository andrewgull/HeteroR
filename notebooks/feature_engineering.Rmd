...
features_filtered$n_repeats_strain_min100 <- ifelse(is.na(features_filtered$n_repeats_strain_min100), 0, features_filtered$n_repeats_strain_min100)
...
features_filtered$n_repeats_strain_min500 <- ifelse(is.na(features_filtered$n_repeats_strain_min500), 0, features_filtered$n_repeats_strain_min500)
...
features_filtered$n_repeats_strain_min1000 <- ifelse(is.na(features_filtered$n_repeats_strain_min1000), 0, features_filtered$n_repeats_strain_min1000)
...
## Add median repeat length per strain
...
features_filtered <- left_join(features_filtered, med_rep_len, by="strain") %>% relocate(median_repeat_length, .before = "AR_length")
```
...
```{r}
med_rep_len_min100 <- features_filtered %>% 
  select(strain, repeat_length) %>% 
  filter(repeat_length >= 100) %>% 
  distinct() %>% 
  group_by(strain) %>% 
...
features_filtered <- left_join(features_filtered, med_rep_len_min100, by="strain") %>% relocate(median_repeat_length_m100, .before = "AR_length")
```
...
```{r}
med_rep_len_min500 <- features_filtered %>% 
  select(strain, repeat_length) %>% 
  filter(repeat_length >= 500) %>% 
  distinct() %>% 
  group_by(strain) %>% 
...
features_filtered <- left_join(features_filtered, med_rep_len_min500, by="strain") %>% relocate(median_repeat_length_m500, .before = "AR_length")
```

155 observations

## Add median AR length
...
features_filtered <- left_join(features_filtered, med_ar_len, by="strain") %>% relocate(median_AR_length, .before = "spans_center")
```
...
features_filtered <- left_join(features_filtered, med_ar_len_m100, by="strain") %>% relocate(median_AR_length_m100, .before = "spans_center")
```
...
```{r}
med_ar_len_m500 <- features_filtered %>%
  filter(repeat_length >= 500) %>% 
  select(strain, AR_length) %>% 
  distinct() %>% 
  group_by(strain) %>% 
...
features_filtered <- left_join(features_filtered, med_ar_len_m500, by="strain") %>% relocate(median_AR_length_m500, .before = "spans_center")
```
...
Let's have a closer look at ampicillins only.

From Features I take all strains tested for AMP resistance and plot them.

Undefined resistance will be removed.

Resistance "I" (intermediate) will also be removed.

**OBS!** Look at presence of a proper resistance gene! (a beta-lactamase gene)
...
```{r}
betaL_n_genes <- features_amp %>% filter(grepl("beta-lactamase", AMR.Gene.Family)) %>% select(strain, record_id) %>% distinct() %>% group_by(strain) %>% summarise(N=n())
...
```{r, eval=FALSE}
# check if some rows don't have beta-lactamase
# sum(grepl("beta-lactamase", features_amp$AMR.Gene.Family))

features_amp$contains_beta_lactamase <- ifelse(grepl("beta-lactamase", features_amp$AMR.Gene.Family), "yes", "no")
features_amp <- features_amp %>% relocate(contains_beta_lactamase, .before="AB")

features_amp_strain <- select(features_amp, strain, AB, contains_beta_lactamase) %>% distinct() %>% group_by(strain) %>% summarise(N=n())
```
...
plot.rl.all.amp <- ggplot(features_amp %>% 
                            select(record_id, repeat_length, AB, resistance, spans_center) %>% 
                            filter(!is.na(resistance), !is.na(spans_center), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, repeat_length)) + 
...
plot.rl.100.amp <- ggplot(features_amp %>% 
                            select(record_id, repeat_length, AB, resistance, spans_center) %>% 
                            filter(!is.na(resistance), !is.na(spans_center), resistance != "I") %>% 
                            filter(repeat_length >= 100) %>% 
                            distinct(), 
  ...
  facet_grid(. ~ spans_center, labeller = label_both) +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ ylab("repeat length, min 100 bp")


plot.rl.500.amp <- ggplot(features_amp %>% 
                            select(record_id, repeat_length, AB, resistance, spans_center) %>% 
                            filter(!is.na(resistance), !is.na(spans_center), resistance != "I") %>% 
                            filter(repeat_length >= 500) %>% 
                            distinct(), 
  ...
  facet_grid(. ~ spans_center, labeller = label_both) +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ ylab("repeat length, min 500 bp")

plot.rl.m100.amp <- ggplot(features_amp %>% 
                            select(record_id, repeat_length, AB, resistance, spans_center) %>% 
                            filter(!is.na(resistance), !is.na(spans_center), resistance != "I") %>% 
                            filter(repeat_length < 100) %>% 
                            distinct(), 
...
```{r, fig.height=9, fig.width=9}
plot.ar.all.amp <- ggplot(features_amp %>% 
...
plot.mar.all.amp <- ggplot(features_amp %>% 
...
plot.mar.100.amp <- ggplot(features_amp %>% 
...
plot.mar.500.amp <- ggplot(features_amp %>% 
