...
n.beta.lac <- f_amp_filt %>% group_by(strain) %>% summarise(n_beta_lac=sum(is_beta_lac))

# join with features_amp
features_amp <- left_join(features_amp, n.beta.lac, by="strain") %>%
  relocate(AMR.Gene.Family, n_beta_lac, .before="repeat_length")

features_amp <- left_join(features_amp, f_amp_filt %>% select(record_id, is_beta_lac), by="record_id") %>% 
  relocate(is_beta_lac, .before="n_beta_lac")
```
...
features_amp <- left_join(features_amp, repeat_counts_strain, by="strain") 
...
# locate
features_amp <- features_amp %>% relocate(n_repeats_strain, n_repeats_gene, .before="ORF_ID")
...
# join 
features_amp <- left_join(features_amp, repeat_counts_strain_100, by="strain")
...
features_amp$n_repeats_strain_min100 <- ifelse(is.na(features_amp$n_repeats_strain_min100), 0, features_amp$n_repeats_strain_min100)

# locate
features_amp <- features_amp %>% relocate(n_repeats_strain_min100, n_repeats_gene_min100, .before="ORF_ID")
...
# join 
features_amp <- left_join(features_amp, repeat_counts_strain_500, by="strain")
...
features_amp$n_repeats_strain_min500 <- ifelse(is.na(features_amp$n_repeats_strain_min500), 0, features_amp$n_repeats_strain_min500)

# locate
features_amp <- features_amp %>% relocate(n_repeats_strain_min500, n_repeats_gene_min500, .before="ORF_ID")

```

### Add repeat count, min length 1000 bp

```{r}
repeat_counts_strain_1000 <-  features_amp %>% 
  filter(is_beta_lac) %>% 
  filter(repeat_length >= 1000) %>% 
  group_by(strain) %>% 
  summarise("n_repeats_strain_min1000"=n())

repeat_counts_gene_1000 <-  features_amp %>% 
  filter(is_beta_lac) %>%  
  filter(repeat_length >= 1000) %>% 
  group_by(record_id) %>% 
...
# join 
features_amp <- left_join(features_amp, repeat_counts_strain_1000, by="strain")
features_amp <- left_join(features_amp, repeat_counts_gene_1000, by="record_id")

# not all beta-lac genes/strains have repeats longer than 500
# they are NA
# turn these NAs to 0s
features_amp$n_repeats_gene_min1000 <- ifelse(is.na(features_amp$n_repeats_gene_min1000), 0, features_amp$n_repeats_gene_min1000)
features_amp$n_repeats_strain_min1000 <- ifelse(is.na(features_amp$n_repeats_strain_min1000), 0, features_amp$n_repeats_strain_min1000)

# locate
features_amp <- features_amp %>% relocate(n_repeats_strain_min1000, n_repeats_gene_min1000, .before="ORF_ID")
...
rep_len_sum <- features_amp %>% filter(is_beta_lac) %>% 
  filter(spans_center=="yes") %>%
  group_by(record_id) %>%
...
## Median repeat length per strain
...
features_amp <- left_join(features_amp, med_rep_len, by="strain") %>% 
  relocate(median_repeat_length, .before = "AR_length")
```
...
```{r}
med_rep_len_min100 <- features_amp %>% 
  filter(is_beta_lac) %>% 
  select(strain, repeat_length) %>% 
  filter(repeat_length >= 100) %>% 
  distinct() %>% 
  group_by(strain) %>% 
  summarize(median_repeat_length_m100=median(repeat_length, na.rm = T))

features_amp <- left_join(features_amp, med_rep_len_min100, by="strain") %>% relocate(median_repeat_length_m100, .before = "AR_length")
```
...
```{r}
med_rep_len_min500 <- features_amp %>% 
  filter(is_beta_lac) %>% 
  select(strain, repeat_length) %>% 
  filter(repeat_length >= 500) %>% 
  distinct() %>% 
  group_by(strain) %>% 
  summarize(median_repeat_length_m500=median(repeat_length, na.rm = T))

features_amp <- left_join(features_amp, med_rep_len_min500, by="strain") %>% relocate(median_repeat_length_m500, .before = "AR_length")
```
...
med_ar_len <- features_amp %>% 
  ...
  distinct() %>% 
  group_by(strain) %>% 
  summarize(median_AR_length=median(AR_length, na.rm = T))

features_amp <- left_join(features_amp, med_ar_len, by="strain") %>% relocate(median_AR_length, .before = "spans_center")
```
...
med_ar_len_m100 <- features_amp %>%
  ...
  distinct() %>% 
  group_by(strain) %>% 
  summarize(median_AR_length_m100=median(AR_length, na.rm = T))

features_amp <- left_join(features_amp, med_ar_len_m100, by="strain") %>% relocate(median_AR_length_m100, .before = "spans_center")
```
...
```{r}
med_ar_len_m500 <- features_amp %>%
  filter(is_beta_lac) %>% 
  filter(repeat_length >= 500) %>% 
  select(strain, AR_length) %>% 
  distinct() %>% 
  group_by(strain) %>% 
  summarize(median_AR_length_m500=median(AR_length, na.rm = T))

features_amp <- left_join(features_amp, med_ar_len_m500, by="strain") %>% relocate(median_AR_length_m500, .before = "spans_center")
```
...
ggplot(n.beta.lac, aes(resistance, n_beta_lac))+
...
```{r}
feat_plot <- features_amp %>% 
  select(strain, n_plasmids, resistance) %>% 
  distinct() %>% 
  filter(resistance != "I", !is.na(resistance))

ggplot(feat_plot, aes(resistance, n_plasmids))+
...
# we don't consider repeats of non beta-lactamase genes
###########################################
feat_plot <- features_amp %>% 
  filter(is_beta_lac) %>% 
  select(record_id, repeat_length, AB, resistance, spans_center, is_beta_lac) %>% 
  filter(!is.na(resistance), !is.na(spans_center), resistance != "I") %>% 
  distinct()

plot.rl.all.amp <- ggplot(feat_plot, aes(resistance, repeat_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T, varwidth = T)+
  coord_trans(y="log") +
  facet_grid(. ~ spans_center, labeller = label_both) +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("repeat length") +
  ggtitle(label="Repeat length distribution among strains tested for AMP resistance", subtitle = "repeats around beta-lactamase genes only")
...
plot.rl.100.amp <- ggplot(feat_plot, aes(resistance, repeat_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  ...
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ ylab("repeat length")+
  ggtitle("Repeat length distribution among strains tested for AMP resistance", subtitle = "minimal repeat length 100 bp, repeats around beta-lactamase genes only")
...
plot.rl.500.amp <- ggplot(feat_plot, aes(resistance, repeat_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = F, varwidth = T)+
  coord_trans(y="sqrt") +
  facet_grid(. ~ spans_center, labeller = label_both) +
...
plot.rl.m100.amp <- ggplot(feat_plot, aes(resistance, repeat_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T, varwidth = T)+
  coord_trans(y="sqrt") +
  facet_grid(. ~ spans_center, labeller = label_both) +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ ylab("repeat length, max 100 bp")

ggarrange(plot.rl.all.amp, plot.rl.100.amp, plot.rl.500.amp, plot.rl.m100.amp, 
          ncol=1, nrow=4, 
...
# all repeat length plot
plot.mrl.all.amp <- ggplot(features_amp %>% 
                            select(strain, median_repeat_length, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, median_repeat_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T, varwidth = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("median repeat length") +
  ggtitle("Median repeat length per strain")

plot.mrl.100.amp <- ggplot(features_amp %>% 
                            select(strain, median_repeat_length_m100, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, median_repeat_length_m100)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = F, varwidth = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ ylab("Median repeat length, min 100 bp")


plot.mrl.500.amp <- ggplot(features_amp %>% 
                            select(strain, median_repeat_length_m500, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, median_repeat_length_m500)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = F, varwidth = T)+
  coord_trans(y="log") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ ylab("Median repeat length, min 500 bp")

ggarrange(plot.mrl.all.amp, plot.mrl.100.amp, plot.mrl.500.amp, 
          ncol=1, nrow=3, 
          common.legend = T, legend = "bottom")
```
...
plot.rc.s.all.amp <- ggplot(features_amp %>% 
                            select(strain, n_repeats_strain, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, n_repeats_strain)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("N") +
...
plot.rc.s.100.amp <- ggplot(features_amp %>% 
                            select(strain, n_repeats_strain_min100, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, n_repeats_strain_min100)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("")+
  ggtitle("Repeat count per strain, min 100 bp")


plot.rc.s.500.amp <- ggplot(features_amp %>% 
                            select(strain, n_repeats_strain_min500, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, n_repeats_strain_min500)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("N")+
  ggtitle("Repeat count per strain, min 500 bp")

plot.rc.s.1000.amp <- ggplot(features_amp %>% 
                            select(strain, n_repeats_strain_min1000, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, n_repeats_strain_min1000)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("")+
  ggtitle("Repeat count per strain, min 1000 bp")

ggarrange(plot.rc.s.all.amp, plot.rc.s.100.amp, plot.rc.s.500.amp, plot.rc.s.1000.amp, 
          ncol=2, nrow=2, 
          common.legend = T, legend = "bottom")
```
...
ggarrange(plot.rc.g.all.amp, plot.rc.g.100.amp, plot.rc.g.500.amp, plot.rc.g.1000.amp, 
          ncol=2, nrow=2, 
          common.legend = T, legend = "bottom")
```
...
```{r, fig.height=9, fig.width=9}
plot.ar.all.amp <- ggplot(features_amp %>% filter(is_beta_lac) %>% 
                            select(record_id, AR_length, AB, resistance, spans_center) %>% 
                            filter(!is.na(resistance), !is.na(spans_center), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, AR_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  ...
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("Length") +
  ggtitle("AR size per repeat pair")


plot.mar.all.amp <- ggplot(features_amp %>% filter(is_beta_lac) %>% 
                            select(strain, median_AR_length, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, median_AR_length)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T, varwidth = T)+
  coord_trans(y="sqrt")+
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("Median")+
  ggtitle("Median AR size per strain")


plot.mar.100.amp <- ggplot(features_amp %>% filter(is_beta_lac) %>% 
                            select(strain, median_AR_length_m100, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, median_AR_length_m100)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = F, varwidth = T)+
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("Median")+
  ggtitle("Median AR size per strain, repeat min 100 bp")

plot.mar.500.amp <- ggplot(features_amp %>% filter(is_beta_lac) %>% 
                            select(strain, median_AR_length_m500, AB, resistance) %>% 
                            filter(!is.na(resistance), resistance != "I") %>% 
                            distinct(), 
                          aes(AB, median_AR_length_m500)) + 
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = T, varwidth = T)+
  coord_trans(y="sqrt") +
  scale_fill_brewer(palette = "Set2") +
  xlab("")+ylab("")+
  ggtitle("Median AR size per strain, repeat min 500 bp")

ggarrange(plot.ar.all.amp, plot.mar.all.amp, plot.mar.100.amp, plot.mar.500.amp, 
          ncol=2, nrow=2, 
          common.legend = T, legend = "bottom")
```
...
feat_plot <- features_amp %>% 
  select(strain, median_repeat_length, total_repeat_length, median_AR_length, resistance) %>% 
  filter(!is.na(resistance), resistance != "I") %>% 
  distinct()

plot.comb.1 <- ggplot(feat_plot %>% 
                        select(median_AR_length, median_repeat_length, resistance) %>% 
                        distinct(), 
                      aes(median_repeat_length,  median_AR_length)) + 
  geom_point(aes(color=resistance), alpha=0.5)+ 
  coord_trans(x="sqrt", y="sqrt")+
  scale_color_brewer(palette = "Set2")


plot.comb.2 <- ggplot(feat_plot %>% 
                        select(total_repeat_length, median_AR_length, resistance) %>% 
                        distinct(), 
                      aes(total_repeat_length,  median_AR_length)) + 
  geom_point(aes(color=resistance), alpha=0.5)+ 
  coord_trans(x="sqrt", y="sqrt")+
  scale_color_brewer(palette = "Set2")

ggarrange(plot.comb.1, plot.comb.2, ncol=1, nrow=2,  common.legend = T, legend = "bottom")
```
