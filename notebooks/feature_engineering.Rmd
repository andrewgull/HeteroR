...
### Filtering Resistance.Mechanism
...
```{r, fig.width=10, fig.height=5}
ggbox.rc.ab.100 <- ggplot(features %>% 
                            filter(!is.na(resistance)) %>% 
                            filter(resistance != "I") %>% 
                            filter(length>=100), aes(AB, n_repeats_strain)) +
...
```{r, fig.width=10, fig.height=5}
ggbox.rc.ab.500 <- ggplot(features %>% 
                            filter(!is.na(resistance)) %>% 
                            filter(resistance != "I") %>% 
                            filter(length>=500), aes(AB, n_repeats_strain)) +
        geom_boxplot(aes(fill = resistance), notch=F, outlier.alpha = 0.5, outlier.size = 0.5) +
...
```{r, fig.width=10, fig.height=5}
ggbox.rl.ab.100 <- ggplot(features %>% 
                            filter(!is.na(resistance)) %>% 
                            filter(resistance != "I") %>% 
                            filter(length>=100), aes(AB, length)) +
        geom_boxplot(aes(fill = resistance), notch=F, outlier.alpha = 0.5, outlier.size = 0.5) +
        coord_trans(y="log") +
        facet_grid(spans_center ~ .) +
        scale_fill_brewer(palette = "Set2") +
...
```{r, fig.width=10, fig.height=5}
ggbox.rl.ab.500 <- ggplot(features %>% 
                            filter(!is.na(resistance)) %>% 
                            filter(resistance != "I") %>% 
                            filter(length>=500), aes(AB, length)) +
        geom_boxplot(aes(fill = resistance), notch=F, outlier.alpha = 0.5, outlier.size = 0.5) +
        coord_trans(y="log") +
        facet_grid(spans_center ~ .) +
        scale_fill_brewer(palette = "Set2") +
...
```{r, fig.width=10, fig.height=5}
ggbox.arl.ab.100 <- ggplot(features %>% 
                            filter(!is.na(resistance)) %>% 
                            filter(resistance != "I") %>% 
                            filter(length>=100), aes(AB, AR_length)) +
        geom_boxplot(aes(fill = resistance), notch=T, outlier.alpha = 0.5, outlier.size = 0.5) +
        coord_trans(y="sqrt") +
        facet_grid(spans_center ~ .) +
        scale_fill_brewer(palette = "Set2") +
...
```{r}
AMP.100.span <- features %>% filter(!is.na(resistance)) %>%
  filter(resistance != "I") %>%
  filter(length>=100) %>%
  filter(spans_center == "yes") %>%
  filter(AB=="AMP")

fit.arl.AMP <- glm(resistance ~ AR_length, data=AMP.100.span, family = "binomial")
...
rsquared(fit.arl.AMP)
```

## Model R ~ AR*RepCount*RepLength

```{r}
fit.AMP <- glm(resistance ~ AR_length*n_repeats_strain*length*n_plasmids, data=AMP.100.span, family = "binomial")
summary(fit.AMP)
```
...
```{r}
pairs(~ AR_length + n_repeats_strain + length, data=AMP.100.span)
plot(fit.arl.rc.rl.AMP)
```
