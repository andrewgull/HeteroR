...
since nyumber of BL genes is count data.
...
More accurate conclusion from this plot: if number of beta-lactamases is greater than 3 then strain is resistant. The chance a strain with more than 3 BL genes to be susceptible is 0.0143 (1.43%).
...
amp_ampr_gene_presence <- as_tibble(
  as.data.frame(
    map(amr_types, function(x) ifelse(grepl(x, amp_amr_gene_types$AMR.Gene.Family), 1, 0)),
    col.names = amr_types))
...
amp_amr_types_strain <- amp_ampr_gene_presence %>% group_by(strain) %>% summarise_each(funs(sum))
