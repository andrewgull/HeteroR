...
I this table we have `r length(unique(features$record_id))` resistance genes with detected repeats from `r length(unique(features$strain))` strains.
...
Difference between rgi genes record ID and repeat_df record if (= features)
See the code below

```{r}
rgi$record_id <- map_chr(rgi$ORF_ID, function(x) strsplit(x, " ")[[1]][1])
setdiff(rgi$record_id, features$record_id)
setdiff(rgi_filt$record_id, features$record_id)
```
...
But first transform them: there should be a column *record_id* like in
`features`
...
Since RGI results have been filtered, they are much smaller than original features that came from data frames with repeats.
...
counts and AR stay in features, let's make `features_filtered` without
these genes
