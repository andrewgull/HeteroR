...
*OBS!* Since RGI results have been filtered, they are much smaller than original features that came from data frames with repeats.

```{r}
features <- full_join(features, rgi_filt, by="record_id")

features %>% filter(is.na(repeat_length)) %>% select(strain.x, strain.y, record_id, ORF_ID)
