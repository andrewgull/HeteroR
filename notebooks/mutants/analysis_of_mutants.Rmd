...
vars_inside_final %>% 
  ...
  select(strain, contig,  locus_tag, gene_name, product, COG, REF, ALT) %>% 
  left_join(ips, by = "locus_tag") %>% 
  select(-locus_tag) %>% 
  relocate(COG, .after = FunFam) %>% 
    reactable::reactable(
    filterable = TRUE,
    minRows = 10,
    sortable = TRUE,
...
left_join(rel_cov_final, genes_annotation, by = "contig.name") %>%
  ...
  rename("rel.depth.mutant" = mutant,
...
bind_rows(new_inserts, new_inserts2) %>% 
  ...
  select(-c(gap, percent_ID, percent_cov)) %>% 
  relocate(strain, .before = region)
  reactable::reactable(
    filterable = TRUE,
    minRows = 10,
    sortable = TRUE,
