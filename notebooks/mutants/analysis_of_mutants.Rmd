...
Taken from the annotation tables
...
vars_inside_final %>% 
  ...
  select(strain, contig, gene_name, product, locus_tag, COG, REF, ALT) %>% 
    reactable::reactable(
    filterable = TRUE,
    minRows = 10,
    sortable = TRUE,
...
left_join(amp_df, genes_annotation, by = "locus_tag") %>%
  select(strain, contig, strand, start, stop, gene_name, product, COG) %>%
  filter(!is.na(gene_name)) %>%
  reactable::reactable(
    filterable = TRUE,
    minRows = 10,
...
Some contigs are too long for plasmids. Vertival line is at 100 kb mark.

Which genes reside on this contigs?
...
Plasmids with increased CN

```{r}
left_join(rel_cov_final, genes_annotation, by = "contig.name") %>% 
  filter(contig.len < 100000) %>% 
  select(-c(tot.depth, contig.len, avg.depth, genome.part)) %>% 
  pivot_wider(names_from = label, values_from = rel.depth) %>% 
  mutate(mut.increase = (mutant - parent) > 0) %>% 
  filter(mut.increase)
```

Plasmids with decreased CN
...
left_join(rel_cov_final, genes_annotation, by = "contig.name") %>% 
  ...
  select(-c(tot.depth, contig.len, avg.depth, genome.part)) %>% 
  pivot_wider(names_from = label, values_from = rel.depth) %>% 
  mutate(mut.decrease = (mutant - parent) < 0) %>% 
  filter(mut.decrease)
```
...
results_files <- dir(path = results_path, pattern = "*table.txt", recursive = T)
...
new_inserts <- map_dfr(results_files,
         ~ ismapper_table(
           table_file = paste0(results_path, .),
           family_id = select(best_is_representatives, seqID, family)
         ))
...
results_files2 <- dir(path = results_path2, pattern = "*table.txt", recursive = T)

# IS family is present in the file name itself
ismapper_table2 <- function(table_file) {
  # table_file: a file with ISmapper results
  out_df <- read_tsv(table_file, show_col_types = F, col_types = "ccdddccccccdcccdc") 
...
new_inserts2 <-
  map_dfr(results_files2, ~ ismapper_table2(
    table_file = paste0(results_path2, .)))
