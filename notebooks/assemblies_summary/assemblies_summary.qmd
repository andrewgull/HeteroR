...
## Incomplete chromosomes total

```{r}
assemblies %>% 
  filter(Type == "Chromosome", Status == "incomplete")
```
...
workflow/scripts/coverage_cli.py ~/Data/HeteroR/resources/strain_lists/strains_in_data_raw.txt 5131220 ~/Data/HeteroR/results/coverage/coverage_strain_in_data_filt.tsv filtered
...
```{r}
assemblies <- left_join(assemblies, coverage, by="Strain")

write.csv(assemblies, assemblies_output_file, row.names = F)
...
ggplot(assemblies %>% filter(Status2 != "additional_plasmid"), aes(Status2, coverage)) +
...
```{r, fig.width=9, fig.height=4}
ggplot(assemblies %>% filter(Status2 != "additional_plasmid"), aes(Status2, sum_len)) +
...
```{r, fig.width=9, fig.height=4}
ggplot(assemblies %>% filter(Status2 != "additional_plasmid"), aes(Status2, max_len)) +
  geom_boxplot(aes(fill=Type), notch=T, varwidth = T) +
  facet_grid(cols = vars(Type))+
  scale_fill_brewer(palette = "Set2")+
  guides(fill="none")+
...
```{r, fig.width=9, fig.height=4}
ggplot(assemblies %>% filter(Status2 != "additional_plasmid"), aes(Status2, avg_len)) +
  geom_boxplot(aes(fill=Type), notch=T, varwidth = T) +
  facet_grid(cols = vars(Type))+
  scale_fill_brewer(palette = "Set2")+
  guides(fill="none")+
...
```{r, fig.width=9, fig.height=4}
ggplot(assemblies %>% filter(Status2 != "additional_plasmid"), aes(Status2, num_seqs)) +
  geom_boxplot(aes(fill=Type), notch=F, varwidth = T) +
  facet_grid(cols = vars(Type))+
  coord_trans(y="sqrt")+
  scale_fill_brewer(palette = "Set2")+
...
```{r}
ggplot(assemblies %>% filter(Type=="Chromosome" & Status2 == "linear"), aes(coverage))+
...
```{r, message = FALSE}
plasmids_count <- assemblies %>% group_by(Strain, Type) %>% summarise(Count = n()) %>% filter(Type=="Plasmid")
plasmids_count
```
