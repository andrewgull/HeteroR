...
Here I explore features of strains tested as HR and non-HR according to
the most recent update from Karin.
...
Data set `data_strain` has been made earlier in
...
Look at target class distribution in the data set you're working with
(414 strains)
...
Well, 120 strains out of 500 tested are not assembled yet and, out of
414 assembled strains, 47 are not tested...

:(

## Some processing

1.  Add BL genes counts as ordered factor and as binary features (if BL
    \> 3)
...
-   It is useful to predict whether a strains *is* (R) or *turns* (HR)
    resistant, because both outcomes require similar actions from a
    clinician.

On the other hand, it has perfect sense to remove R strains completely:
if our task is to distinguish susceptible strains and heteroresistant
ones.
...
```{r}
skim(data_strain) %>% yank("numeric")
```

All the missing values should be replaced with zeros.
...
```{r, eval=FALSE}
write.csv(data_strain, "shiny/data/features_ptz_strain.csv", row.names = F)
```
...
# without resistance
cor_matrix <- cor(data_strain[,c(5:ncol(data_strain))], use = "pairwise.complete.obs")
corrplot::corrplot(cor_matrix, type = "upper", tl.col = "black", tl.cex = 0.8)
```
...
data_rec <- recipe(resistance ~., data = data_strain) %>%
...
p1 <- 
  data_strain %>% 
  ggplot(aes(x = min.dist.oriC)) + 
  geom_histogram(bins = 30, color = "white", fill = "blue", alpha = 1/3) + 
  ggtitle("Original data (min.dist.oriC)")

p2 <- 
  data_prep$template %>% 
  ggplot(aes(x = min.dist.oriC)) + 
  geom_histogram(bins = 30, color = "white", fill = "red", alpha = 1/3) + 
  ggtitle("Processed data (min.dist.oriC)")

p3 <- data_strain %>% 
  ggplot(aes(x = n.plasmids)) + 
  geom_histogram(bins = 30, color = "white", fill = "blue", alpha = 1/3) + 
  ggtitle("Original data (n.plasmids)")
  
p4 <-
  data_prep$template %>% 
  ggplot(aes(x = n.plasmids)) + 
  geom_histogram(bins = 30, color = "white", fill = "red", alpha = 1/3) + 
  ggtitle("Processed data (n.plasmids)")

p5 <- data_strain %>% 
  ggplot(aes(x = n.rep.total)) + 
  geom_histogram(bins = 30, color = "white", fill = "blue", alpha = 1/3) + 
  ggtitle("Original data (n.rep.total)")

p6 <-
  data_prep$template %>% 
  ggplot(aes(x = n.rep.total)) + 
  geom_histogram(bins = 30, color = "white", fill = "red", alpha = 1/3) + 
  ggtitle("Processed data (n.rep.total)")
...
pca_df <- data_pca$template %>%
  mutate(color = if_else(resistance == "HR", "red", "blue"))
...
```{r, eval=FALSE}
pls_df <- data_pls$template %>%
  mutate(color = if_else(resistance == "HR", "red", "blue"))


plot3d(x = pls_df$PLS1, y = pls_df$PLS2, z = pls_df$PLS3,
       col = pls_df$color,
       type = "s", 
       radius = 0.2,
       xlab = "PLS1",
...
```{r, eval=FALSE}
ica_df <- data_ica$template %>%
  mutate(color = if_else(resistance == "HR", "red", "blue"))


plot3d(x = ica_df$IC1, y = ica_df$IC2, z = ica_df$IC3,
       col = ica_df$color,
       type = "s", 
       radius = 0.1,
       xlab = "IC1",
...
umap_rec_sup <- data_rec %>%
  step_umap(all_numeric_predictors(), outcome = "resistance", num_comp = 10, neighbors = 15)
...
Supervised UMAP look really promising. HR clusters are more compact and
some of them are more distant from non-HR ones.

#### 3D version

```{r, eval=FALSE}
umap_df <- data_umap_sup$template %>%
  mutate(color = if_else(resistance == "HR", "red", "blue"))


plot3d(x = umap_df$UMAP01, y = umap_df$UMAP02, z = umap_df$UMAP03,
       col = umap_df$color,
       type = "s", 
       radius = 0.1,
       xlab = "UMAP1",
...
1.  If number of BL-genes is higher than 3, the strain is more likely to
    be HR/R

2.  There are
    `r nrow(data_strain %>% filter(n.beta.lac == 1, resistance == "HR"))`
    HR strain with only 1 beta-lactamase (which is *ampC*)
...
When the number of BL genes is less or equal 3, most likely the strains
is non-HR

When number of BL genes is greater than 3, quite likely that the strain
is HR or R
...
Variable `n.genes.plasmids` means number of **beta-lactamases** on
plasmids (it was renamed).

Let's check this.

It should always be less or equal than `n.beta.lac`.

How many strains in our data set `n.beta.lac` greater or equal
`n.beta.lac.plasmid`:
`r sum(data_strain$n.beta.lac >= data_strain$n.beta.lac.plasmid)`
...
Starting from 3 bl genes (on plasmids), it seems to be a very good
predictor
...
If a strain has BL genes on chromosome only it's more likely to be
non-HR than HR
...
```{r, eval=F}
vals <- scales::rescale(c(0,1,2,3,4))
o <- order(vals, decreasing = FALSE)
cols <- scales::col_numeric("Blues", domain = NULL)(vals)
colz <- setNames(data.frame(vals[o], cols[o]), NULL)
fig <- plot_ly(z = bl_amr_types_strain_td$N, x = bl_amr_types_strain_td$strain, y = bl_amr_types_strain_td$AMR.type, type = "heatmap", colorscale = colz)
fig
```
