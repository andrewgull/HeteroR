...
I will make two data sets with and without R strains, if with, they will be merged with HR.
...
# data_strain <- data_strain %>% relocate(resistance, .before = "n.plasmids")

# merge R and HR into HR/R
data_strain$resistance.mrg <- str_replace(data_strain$resistance, "^R$", "HR/R")
data_strain$resistance.mrg <- str_replace(data_strain$resistance, "^HR$", "HR/R")

# remove R labels
data_strain$resistance.xcl <- na_if(data_strain$resistance, "R")

# relocate them
data_strain <- data_strain %>% relocate(resistance.mrg, resistance.xcl, .before = "n.plasmids")
...
It's EDA, do it with all testing lables!

```{r}
data_strain <- data_strain %>% select(-c(resistance.mrg, resistance.xcl))
```
