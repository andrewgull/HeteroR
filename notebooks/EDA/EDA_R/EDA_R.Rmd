...
```{r data, cache=F, eval=FALSE, include=FALSE}
...
features_amp_strain_scaled <- relocate(features_amp_strain_scaled, resistance, .before="n.beta.lac.3")
```
...
## Descriptive statistics after imputation and scaling

```{r}
skimr::skim(features_amp_strain_scaled)
```
...
If observation is in cluster 2 then it's resistant
...
```{r, fig.width=8, fig.height=6}
k8 <- kmeans(feat_clust, centers = 8, nstart = 100)
fviz_cluster(k8, data = feat_clust, repel = T, show.clust.cent = F, labelsize = 6, )
```
