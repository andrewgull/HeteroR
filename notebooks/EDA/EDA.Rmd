...
# EDA for AMP-tested strains

From Features I have selected all the strains tested for AMP resistance, repeat-related features have been calculated based on beta-lactamase genes of **all types** (ampC, CTX-M, OXA, TEM, SHV)
...
```{r, fig.width=20, fig.height=20, warning=F, message=F, eval=FALSE}
# use GGally

GGally::ggpairs(features_amp_strain, 
        columns=c(2,4:40), 
...
cor.matrix <- cor(features_amp_strain[,c(2,5:40)], use = "pairwise.complete.obs")
...
### Number of beta-lactamases in Resistant and Susceptible strains
...
Conclusion: if number of beta-lactamases is greater than 3 then strain is most likely resistant. The chance a strain with more than 3 BL genes to be susceptible is 0.0143 (1.43%).
...
# add ampC count to the main features table
...
ggplot(features_amp_strain, aes(resistance, med.tot.rep.len))+
  geom_violin(aes(fill=resistance), alpha=0.7)+
  geom_boxplot(aes(fill=resistance), alpha=0.2, notch = F, varwidth = T)+
  geom_jitter(alpha=0.2, width=0.15, height = 0.1)+
  coord_trans(y="log") +
...
plot.corr.dist.oric <- ggplot(features_amp_strain, aes(min.dist.oriC, med.dist.oriC))+
  geom_point(aes(color=resistance), alpha=0.7)+
  scale_color_brewer(palette = "Set1")+
  guides(fill="none")


ggarrange(plot.min.dist.oric, plot.med.dist.oric, plot.corr.dist.oric, ncol=3, nrow=1, legend = "none")
```

Average distance to oriC is smaller in resistant strains
...
evaluation of all the code chunks here is turned off

### All repeat lengths

Should `distinct()` be used?

```{r, fig.height=6, fig.width=14, eval=FALSE}
...
# EDA for all types of antibiotics

Evaluation of the code chunks below is turned off
...
ggplot(features_filtered %>% 
         ...
         filter(resistance !="I"), aes(AB, n.plasmids)) + 
...
ggplot(features_filtered %>% 
         filter(!is.na(resistance)) %>% 
         filter(resistance !="I"), aes(AB, n.plasmids_alt)) + 
  geom_boxplot(aes(fill=resistance), notch=T, outlier.alpha = 0.5, outlier.size = 0.5) +
  scale_fill_brewer(palette = "Set2") +
  coord_trans(y="sqrt") +
  xlab("")+
