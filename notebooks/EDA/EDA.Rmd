...
### Number of beta-lactamases in R and S strains
...
```{r}
bl_count <- features_amp_strain %>% select(resistance, n.beta.lac, ampC) %>% mutate(non.ampC = n.beta.lac - ampC) %>% select(-n.beta.lac)

bl_count_tidy <- gather(bl_count, key = "gene", value = "n", 2:3) %>% 
  group_by(resistance, gene) %>% summarize(sum=sum(n))


ggplot(bl_count_tidy, aes(sum)) +
  geom_bar(aes(fill=gene), position = "fill", alpha = 0.8)+
